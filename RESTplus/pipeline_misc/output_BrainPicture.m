function [varargout]=output_BrainPicture(AOperation, varargin)
% Show a brain's slice. "output_BrainPicture" can be opened more than one instance like MRIcro, and supports multi-slice, overlay and so on. by Xiao-Wei Song
%Usage: hFig =output_BrainPicture('ShowImage', AFilename, CallBack);
%           output_BrainPicture('Delete', AFigHandle);
%Detailed usage is the code file "output_BrainPicture.m"
%------------------------------------------------------------------------------------------------------------------------------
...
if nargin<1, %No param Launch, 20070918
	...
	if nargout>=1,
		...
		output_BrainPicture('ShowImage','');	%by Default, I show a black brain image
	end
	return; 
end

%Initializitation
global output_BrainPicture_Cfg; %YAN Chao-Gan, 090919. %persistent rp_SliceViewer_Cfg; % run-time persistent config
%if isempty(output_BrainPicture_Cfg), disp('ko'); end
if ~mislocked(mfilename),mlock; end
%For further Debug, 20070915, to make sure the Config variable exist
% if isempty(output_BrainPicture_Cfg), 
	% output_BrainPicture_Cfg =getappdata(0, 'output_BrainPicture_Cfg');
% else
	% setappdata(0, 'output_BrainPicture_Cfg', output_BrainPicture_Cfg);
% end
...
try
	...
	case 'SHOWIMAGE',	%ShowImage
		if nargin==2,
			%output_BrainPicture('ShowImage', theBrainMap); %reho_gui.m 989
			AFilename =varargin{1};
			ACallback ='';		
	    elseif nargin==3,
			%output_BrainPicture('ShowImage', theBrainMap, [theCallback cmdClearVar], 'Power Spectrum');%reho_gui.m 1010
			AFilename =varargin{1};
			ACallback =varargin{2};		
		else
			error(sprintf('Usage: hFig =output_BrainPicture(''ShowImage'', AFilename); \n\t hFig =output_BrainPicture(''ShowImage'', AFilename, ACallback);')); 
		end
	    
		%Let current handle of the figure be a GUID for the current SliceViewer
		% theFig =ExistDisplayFigure(output_BrainPicture_Cfg, AFilename);
		% isExistFig =rp_misc( 'ForceCheckExistFigure' , theFig);	%Force check whether the figure exist
		% if ~isExistFig
			%the specific image didn't exist, so I create one
			output_BrainPicture_Cfg.Config(1+GetDisplayCount(output_BrainPicture_Cfg)) =InitControls(AFilename, ACallback);
			%To Force display following the end of this if-clause
			theFig =output_BrainPicture_Cfg.Config(GetDisplayCount(output_BrainPicture_Cfg)).hFig;
			
			%I don't display the information defaultly
			ToggleInfoDisplay(output_BrainPicture_Cfg.Config(GetDisplayCount(output_BrainPicture_Cfg)));
		% end
		figure(theFig);	
		varargout{1} =theFig;
	...
	case 'UPDATECALLBACK', 		%UpdateCallback
		...
		elseif nargin==4, 
			...
			error('Usage: output_BrainPicture(''UpdateCallback'', AFigureHandle, ACallback, ACallbackCaption);'); 
		end
		theCardinal =ExistViewer(output_BrainPicture_Cfg, AFigHandle);
		if theCardinal>0,
			output_BrainPicture_Cfg.Config(theCardinal) =UpdateCallback(output_BrainPicture_Cfg.Config(theCardinal), ACallback, ACallbackCaption);
			ResizeFigure(output_BrainPicture_Cfg.Config(theCardinal));
		end
		
		
	case 'DELETE', % Delete specific figure
		if nargin~=2, error('Usage: output_BrainPicture(''Delete'', AFigHandle);'); end
		AFigHandle =varargin{1};
		output_BrainPicture_Cfg =DeleteFigure(output_BrainPicture_Cfg, AFigHandle);
		
		if ~GetDisplayCount(output_BrainPicture_Cfg),
			output_BrainPicture('QuitAllSliceViewer');
		end
		
	case {'CLICKPOSITION', 'SETPOSITION'},		%ClickPosition	%SetPosition
		if nargin~=2, error('Usage: output_BrainPicture(''ClickPosition'', AFigHandle); or output_BrainPicture(''SetPosition'', AFigHandle);'); end
		AFigHandle =varargin{1};
		theCardinal =ExistViewer(output_BrainPicture_Cfg, AFigHandle);
		if theCardinal>0
			if strcmpi(AOperation, 'ClickPosition'),	% for Mouse click
				if strcmpi(output_BrainPicture_Cfg.Config(theCardinal).ViewMode, 'Orthogonal'),
					output_BrainPicture_Cfg.Config(theCardinal) =ClickPositionCrossHair(output_BrainPicture_Cfg.Config(theCardinal));		
				elseif strcmpi(output_BrainPicture_Cfg.Config(theCardinal).ViewMode, 'Sagittal'),
					output_BrainPicture_Cfg.Config(theCardinal) =ClickPositionInSagittalMode(output_BrainPicture_Cfg.Config(theCardinal));
				elseif strcmpi(output_BrainPicture_Cfg.Config(theCardinal).ViewMode, 'Coronal'),
					output_BrainPicture_Cfg.Config(theCardinal) =ClickPositionInCoronalMode(output_BrainPicture_Cfg.Config(theCardinal));
				elseif strcmpi(output_BrainPicture_Cfg.Config(theCardinal).ViewMode, 'Transverse'),
					output_BrainPicture_Cfg.Config(theCardinal) =ClickPositionInTransverseMode(output_BrainPicture_Cfg.Config(theCardinal));
				end
			elseif strcmpi(AOperation, 'SetPosition'), % for [x y z] manual input
				output_BrainPicture_Cfg.Config(theCardinal) =SetPositionCrossHair(output_BrainPicture_Cfg.Config(theCardinal));
			end		
					
			
			%If yoke, then update all yoked image
			isSelfYoked =get(output_BrainPicture_Cfg.Config(theCardinal).hYoke, 'Value');
			if isSelfYoked ,
				theDistanceFromOrigin =(output_BrainPicture_Cfg.Config(theCardinal).LastPosition -output_BrainPicture_Cfg.Config(theCardinal).Origin) .* output_BrainPicture_Cfg.Config(theCardinal).VoxelSize;
				%Dawnsong 20071102 Revise to make sure the left image/Right brain is +
				theDistanceFromOrigin =[-1, 1, 1].* theDistanceFromOrigin;
				
				for x=1:GetDisplayCount(output_BrainPicture_Cfg), 
					if x~=theCardinal ,
						isYoked =get(output_BrainPicture_Cfg.Config(x).hYoke, 'Value');
						if isYoked, 							
							output_BrainPicture('SetPhysicalPosition', output_BrainPicture_Cfg.Config(x).hFig, theDistanceFromOrigin);
						end
					end
				end
			end	
			...
			output_BrainPicture('RunCallback', AFigHandle);			
		end
		
	case 'SETPHYSICALPOSITION',		%SetPhysicalPosition, for Yoke, the physical position(mm) from the origin
		if nargin~=3, error('Usage: output_BrainPicture(''SetPhysicalPosition'', AFigHandle, ADistanceFromOrigin);'); end
		AFigHandle =varargin{1};
		...
		if theCardinal>0,
			...
			output_BrainPicture_Cfg.Config(theCardinal) =SetDistanceFromOrigin(output_BrainPicture_Cfg.Config(theCardinal), ADistanceFromOrigin);
			
			%Execute the Callback
			output_BrainPicture('RunCallback', AFigHandle);	
		end		
	case 'RUNCALLBACK', 			%RunCallback	, 20070625
		if nargin~=2, error('Usage: output_BrainPicture(''RunCallback'', AFigHandle);'); end
		AFigHandle =varargin{1};	
		theCardinal =ExistViewer(output_BrainPicture_Cfg, AFigHandle);
		if theCardinal>0		
			% Run the Callback
			if ~isempty(output_BrainPicture_Cfg.Config(theCardinal).Callback) 
				if ischar(output_BrainPicture_Cfg.Config(theCardinal).Callback),
					eval(output_BrainPicture_Cfg.Config(theCardinal).Callback); %run callback for caller
				elseif isa(output_BrainPicture_Cfg.Config(theCardinal).Callback, 'function_handle')
					%I give 2 parameters, 20070624
					%1) position: X, Y , Z, 
					%2) and 3-dim: XSize, YSize, ZSize
					thePos =output_BrainPicture_Cfg.Config(theCardinal).LastPosition;
					theSize=size(output_BrainPicture_Cfg.Config(theCardinal).Volume);
					output_BrainPicture_Cfg.Config(theCardinal).Callback(thePos, theSize);
				end
			end		
		end
	...
	case 'GETPOSITION', 		%GetPosition, return Absolute	position where current cross-hair stay
		if nargin~=2, error('Usage: Position =output_BrainPicture(''GetPosition'', AFigHandle);'); end
		AFigHandle =varargin{1};
		theCardinal =ExistViewer(output_BrainPicture_Cfg, AFigHandle);
		if theCardinal>0
			varargout{1}=output_BrainPicture_Cfg.Config(theCardinal).LastPosition;	 
		end
		
	case 'GETPHYSICALPOSITION', 		%GetPhysicalPosition return Coordinate .* VoxelSize	(mm)
		if nargin~=2, error('Usage: DistanceFromOrigin =output_BrainPicture(''GetPhysicalPosition'', AFigHandle);'); end
		AFigHandle =varargin{1};
		theCardinal =ExistViewer(output_BrainPicture_Cfg, AFigHandle);
		if theCardinal>0
			thePosition =output_BrainPicture_Cfg.Config(theCardinal).LastPosition;	 
			theOrigin  	=output_BrainPicture_Cfg.Config(theCardinal).Origin;
			theVoxelSize=output_BrainPicture_Cfg.Config(theCardinal).VoxelSize;
	...
	case 'SETMESSAGE',			%SetMessage, update the Message, I don't contain the Copyright any more , 20070915
		if nargin~=3, error('Usage: output_BrainPicture(''SetMessage'', AFigHandle, AMessage);'); end
		AFigHandle 	=varargin{1};
		AMessage	=varargin{2};
		theCardinal =ExistViewer(output_BrainPicture_Cfg, AFigHandle);
		if theCardinal>0
			output_BrainPicture_Cfg.Config(theCardinal).Message =AMessage;
			SetMessage(output_BrainPicture_Cfg.Config(theCardinal));
			ResizeFigure(output_BrainPicture_Cfg.Config(theCardinal));
		end	
		
	case 'MAGNIFY', 		%Magnify, x0.5 -x1 -x2 -x3 -x4 -x5 -x6
		if nargin~=2, error('Usage: output_BrainPicture(''Magnify'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(output_BrainPicture_Cfg, AFigHandle);
		if theCardinal>0
			output_BrainPicture_Cfg.Config(theCardinal) =SetImage(output_BrainPicture_Cfg.Config(theCardinal));			
		end	
		
	case 'QUITALLSLICEVIEWER',		%QuitAllSliceViewer
		if nargin~=1, error('Usage: output_BrainPicture(''QuitAllSliceViewer'');'); end
		for x=GetDisplayCount(output_BrainPicture_Cfg):-1:1, % DawnSong, revised 20070625
			output_BrainPicture('DELETE', output_BrainPicture_Cfg.Config(x).Filename);
		end
		if mislocked(mfilename),munlock; end
		clear output_BrainPicture_Cfg;
		
	case 'MONTAGE', 		%Montage	
		if nargin~=2, error('Usage: output_BrainPicture(''Montage'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(output_BrainPicture_Cfg, AFigHandle);
		if theCardinal>0,
			...
			def	={	int2str(output_BrainPicture_Cfg.Config(theCardinal).Montage.Across), ...
					int2str(output_BrainPicture_Cfg.Config(theCardinal).Montage.Down), ...
					int2str(output_BrainPicture_Cfg.Config(theCardinal).Montage.Spacing), ...
					int2str(output_BrainPicture_Cfg.Config(theCardinal).Montage.WantLabel)};
			answer =inputdlg(prompt, 'Montage Set', 1, def);
			if numel(answer)==4,
				theVal =abs(round(str2num(answer{1})));
				if theVal==0, theVal=1; end
				output_BrainPicture_Cfg.Config(theCardinal).Montage.Across = theVal;
				theVal =abs(round(str2num(answer{2})));
				if theVal==0, theVal=1; end
				output_BrainPicture_Cfg.Config(theCardinal).Montage.Down =theVal;
				output_BrainPicture_Cfg.Config(theCardinal).Montage.Spacing = abs(round(str2num(answer{3})));
				output_BrainPicture_Cfg.Config(theCardinal).Montage.WantLabel = abs(round(str2num(answer{4})));
     ...
     case 'DF'
         ...
         theCardinal =ExistViewer(output_BrainPicture_Cfg, AFigHandle);
         if theCardinal>0,
             if isfield(output_BrainPicture_Cfg.Config(theCardinal).Overlay.Header,'descrip')
                 headinfo=output_BrainPicture_Cfg.Config(theCardinal).Overlay.Header.descrip; %dong 090921AConfig.Df.Ttest=0;
                 ...
                 if exist('testFlag')
                     if ~isempty(testFlag)
                         if exist('testDf')
                             if testFlag == 'T'
                                 output_BrainPicture_Cfg.Config(theCardinal).Df.Ttest=testDf;
                             elseif testFlag == 'F'
                                 output_BrainPicture_Cfg.Config(theCardinal).Df.Ftest=testDf;
                             elseif testFlag == 'R'
                                 output_BrainPicture_Cfg.Config(theCardinal).Df.Rtest=testDf;
                             elseif testFlag == 'Z'
                                 output_BrainPicture_Cfg.Config(theCardinal).Df.Ztest=testDf;
                             end
                         end % dong 100331 end
                     end
                 end
             ...
             def	={	int2str(output_BrainPicture_Cfg.Config(theCardinal).Df.Ttest), ...
                 int2str(output_BrainPicture_Cfg.Config(theCardinal).Df.Ftest), ...
                 int2str(output_BrainPicture_Cfg.Config(theCardinal).Df.Ztest), ...
                 int2str(output_BrainPicture_Cfg.Config(theCardinal).Df.Rtest), ...
                 };
             options.Resize='on';
             ...
             if numel(answer)==4,
                 theVal =abs(round(str2num(answer{1})));
                 output_BrainPicture_Cfg.Config(theCardinal).Df.Ttest = theVal;
                 theVal =abs(round(str2num(answer{2})));
                 output_BrainPicture_Cfg.Config(theCardinal).Df.Ftest = theVal;
                 theVal =abs(round(str2num(answer{3})));
                 output_BrainPicture_Cfg.Config(theCardinal).Df.Ztest = theVal;
                 theVal =abs(round(str2num(answer{4})));
                 output_BrainPicture_Cfg.Config(theCardinal).Df.Rtest = theVal;
                 SetMessage(output_BrainPicture_Cfg.Config(theCardinal));
             end
         end;
	case 'ORTHOGONALVIEW', 		%OrthogonalView
		if nargin~=2, error('Usage: output_BrainPicture(''OrthogonalView'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(output_BrainPicture_Cfg, AFigHandle);
		if theCardinal>0,
			output_BrainPicture_Cfg.Config(theCardinal).ViewMode ='Orthogonal';
			output_BrainPicture_Cfg.Config(theCardinal) =SetImage(output_BrainPicture_Cfg.Config(theCardinal));
		end;	
		
	case 'TRANSVERSEVIEW', 		%TransverseView
		if nargin~=2, error('Usage: output_BrainPicture(''TransverseView'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(output_BrainPicture_Cfg, AFigHandle);
		if theCardinal>0,
			output_BrainPicture_Cfg.Config(theCardinal).ViewMode ='Transverse';
			output_BrainPicture_Cfg.Config(theCardinal) =SetImage(output_BrainPicture_Cfg.Config(theCardinal));
		end;
			
	case 'SAGITTALVIEW', 		%SagittalView
		if nargin~=2, error('Usage: output_BrainPicture(''SagittalView'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(output_BrainPicture_Cfg, AFigHandle);
		if theCardinal>0,
			output_BrainPicture_Cfg.Config(theCardinal).ViewMode ='Sagittal';
			output_BrainPicture_Cfg.Config(theCardinal) =SetImage(output_BrainPicture_Cfg.Config(theCardinal));
		end;

	case 'CORONALVIEW', 		%CoronalView
		if nargin~=2, error('Usage: output_BrainPicture(''CoronalView'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(output_BrainPicture_Cfg, AFigHandle);
		if theCardinal>0,
			output_BrainPicture_Cfg.Config(theCardinal).ViewMode ='Coronal';
			output_BrainPicture_Cfg.Config(theCardinal) =SetImage(output_BrainPicture_Cfg.Config(theCardinal));
		end;

	case 'REPAINT',		%Repaint	
		if nargin~=2, error('Usage: output_BrainPicture(''Repaint'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(output_BrainPicture_Cfg, AFigHandle);
		if theCardinal>0,		
			output_BrainPicture_Cfg.Config(theCardinal) =SetImage(output_BrainPicture_Cfg.Config(theCardinal));
	...
	case 'MNI/TALAIRACH',		%MNI/Talairach
		...
		theCardinal =ExistViewer(output_BrainPicture_Cfg, AFigHandle);
		if theCardinal>0,		
			Transforming_MNI_Talairach(output_BrainPicture_Cfg.Config(theCardinal));
			output_BrainPicture('SetPosition', AFigHandle);
		end;
		
	case 'CHANGEUNDERLAY', 		%ChangeUnderlay
		if nargin~=2, error('Usage: output_BrainPicture(''ChangeUnderlay'', AFigHandle);'); end
		AFigHandle 	=varargin{1};
		theCardinal =ExistViewer(output_BrainPicture_Cfg, AFigHandle);
		if theCardinal>0,
			%Get the changed Underlay file
			theNewUnderlay =get(output_BrainPicture_Cfg.Config(theCardinal).hUnderlayFile, 'String');
			% if exist(theNewUnderlay,'file')==2,
			% if ~all(isspace(theNewUnderlay)),	%20070918
				%Set the current underlay
				output_BrainPicture_Cfg.Config(theCardinal).Filename =theNewUnderlay;
				output_BrainPicture_Cfg.Config(theCardinal) =InitUnderlay(output_BrainPicture_Cfg.Config(theCardinal));
                % YAN Chao-Gan and DONG Zhang-Ye, 100401.
                if all(output_BrainPicture_Cfg.Config(theCardinal).VoxelSize~=[1 1 1])
                ...
                if isfield(output_BrainPicture_Cfg.Config(theCardinal).Header,'mat')
                    matinfo=output_BrainPicture_Cfg.Config(theCardinal).Header.mat;
                ...
                output_BrainPicture_Cfg.Config(theCardinal) =SetImage(output_BrainPicture_Cfg.Config(theCardinal));
                set(output_BrainPicture_Cfg.Config(theCardinal).hUnderlayFile, 'String', theNewUnderlay, 'TooltipString', theNewUnderlay);
			% end
		end;
		
	case 'UNDERLAYSELECTION', 		%UnderlaySelection	
		if nargin~=2, error('Usage: output_BrainPicture(''UnderlaySelection'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(output_BrainPicture_Cfg, AFigHandle);
		if theCardinal>0,			
			...
			if any(filename~=0) && ischar(filename) && length(filename)>4 ,	% not canceled and legal			
				...
				set(output_BrainPicture_Cfg.Config(theCardinal).hUnderlayFile, 'String', theBrainMap);
				output_BrainPicture('ChangeUnderlay', AFigHandle);
			end			
		end;
	case 'CLICKRECENTUNDERLAY', 	%ClickRecentUnderlay
		if nargin~=2, error('Usage: output_BrainPicture(''ClickRecentUnderlay'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(output_BrainPicture_Cfg, AFigHandle);
		if theCardinal>0,			
			theIndex =get(output_BrainPicture_Cfg.Config(theCardinal).hUnderlayRecent, 'Value');
			if theIndex>1,
				theBrainMap =output_BrainPicture_Cfg.Config(theCardinal).Recent.Underlay{theIndex-1};
				set(output_BrainPicture_Cfg.Config(theCardinal).hUnderlayFile, 'String', theBrainMap);
				set(output_BrainPicture_Cfg.Config(theCardinal).hUnderlayRecent, 'Value', 1);
				output_BrainPicture('ChangeUnderlay', AFigHandle);				
			end
		end;
		
	case 'CHANGEOVERLAY', 		%ChangeOverlay
		if nargin~=2, error('Usage: output_BrainPicture(''ChangeOverlay'', AFigHandle);'); end
		AFigHandle 	=varargin{1};
		theCardinal =ExistViewer(output_BrainPicture_Cfg, AFigHandle);
		if theCardinal>0,
			%Get the changed Overlay file
			theNewOverlay =get(output_BrainPicture_Cfg.Config(theCardinal).hOverlayFile, 'String');
			% if exist(theNewOverlay,'file')==2,
			% if ~all(isspace(theNewOverlay)),
				%Set the current Overlay
                output_BrainPicture_Cfg.Config(theCardinal) =LoadOverlay(output_BrainPicture_Cfg.Config(theCardinal), theNewOverlay);
                % YAN Chao-Gan and DONG Zhang-Ye, 100401.
                if isfield(output_BrainPicture_Cfg.Config(theCardinal).Overlay.Header,'mat')
                    matinfo=output_BrainPicture_Cfg.Config(theCardinal).Overlay.Header.mat;
                    if ~(matinfo(1,2)==0&&matinfo(1,3)==0&&matinfo(2,1)==0&&matinfo(2,3)==0&&matinfo(3,1)==0&&matinfo(3,2)==0&&matinfo(4,1)==0&&matinfo(4,2)==0&&matinfo(4,3)==0);
                ...
                set(output_BrainPicture_Cfg.Config(theCardinal).hSeeOverlay, 'Value', 1);
				output_BrainPicture_Cfg.Config(theCardinal) =SetImage(output_BrainPicture_Cfg.Config(theCardinal));
				set(output_BrainPicture_Cfg.Config(theCardinal).hOverlayFile, 'String', theNewOverlay, 'TooltipString', theNewOverlay);
			% end
		end;
	...
	case 'OVERLAYSELECTION', 		%OverlaySelection	
		...
		if theCardinal>0,		
			...
			if any(filename~=0) && ischar(filename) && length(filename)>4 ,	% not canceled and legal			
				...
				theBrainMap =[pathname filename];
				set(output_BrainPicture_Cfg.Config(theCardinal).hOverlayFile, 'String', theBrainMap);
                set(output_BrainPicture_Cfg.Config(theCardinal).hEdtPValue, 'String', '1');  %DONG 100118 change the init P value
				output_BrainPicture('ChangeOverlay', AFigHandle);
                if isfield(output_BrainPicture_Cfg.Config(theCardinal).Overlay.Header,'descrip')   %DONG 100118 to read the DF when overlay selected.
                headinfo=output_BrainPicture_Cfg.Config(theCardinal).Overlay.Header.descrip; %dong 090921AConfig.Df.Ttest=0;
               if isfield(output_BrainPicture_Cfg.Config(theCardinal).Overlay.Header,'descrip')
                 headinfo=output_BrainPicture_Cfg.Config(theCardinal).Overlay.Header.descrip; %dong 090921AConfig.Df.Ttest=0;
                 if ~isempty(strfind(headinfo,'{T_['))% dong 100331 begin
	...
	case 'CLICKRECENTOVERLAY', 	%ClickRecentOverlay
		if nargin~=2, error('Usage: output_BrainPicture(''ClickRecentOverlay'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(output_BrainPicture_Cfg, AFigHandle);
		if theCardinal>0,			
			theIndex =get(output_BrainPicture_Cfg.Config(theCardinal).hOverlayRecent, 'Value');
			if theIndex>1,
				theBrainMap =output_BrainPicture_Cfg.Config(theCardinal).Recent.Overlay{theIndex-1};
				set(output_BrainPicture_Cfg.Config(theCardinal).hOverlayFile, 'String', theBrainMap);
				set(output_BrainPicture_Cfg.Config(theCardinal).hOverlayRecent, 'Value', 1);
				output_BrainPicture('ChangeOverlay', AFigHandle);
			end
		end;
		
	case 'OVERLAY_SETTHRDABSVALUE',		%Overlay_SetThrdAbsValue, %Change from the Absolute value Edit control
		if nargin~=2, error('Usage: output_BrainPicture(''Overlay_SetThrdAbsValue'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(output_BrainPicture_Cfg, AFigHandle);
		if theCardinal>0,
			output_BrainPicture_Cfg.Config(theCardinal).Overlay.ValueThrdAbsolute =SetThrdAbsValue(output_BrainPicture_Cfg.Config(theCardinal));
			output_BrainPicture_Cfg.Config(theCardinal) =ThresholdOverlayVolume(output_BrainPicture_Cfg.Config(theCardinal));
			output_BrainPicture_Cfg.Config(theCardinal) =SetImage(output_BrainPicture_Cfg.Config(theCardinal));			
		end	
	case 'OVERLAY_SETTHRDCLUSTERSIZE', 		%Overlay_SetThrdClusterSize
		if nargin~=2, error('Usage: output_BrainPicture(''Overlay_SetThrdClusterSize'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(output_BrainPicture_Cfg, AFigHandle);
		if theCardinal>0,
		 output_BrainPicture_Cfg.Config(theCardinal).Overlay.PNflag=0;
			theConfig =output_BrainPicture_Cfg.Config(theCardinal);
			...
			if numel(answer)==3,
				...
				output_BrainPicture_Cfg.Config(theCardinal) =theConfig;
				output_BrainPicture_Cfg.Config(theCardinal) =ThresholdOverlayVolume(output_BrainPicture_Cfg.Config(theCardinal));
				output_BrainPicture_Cfg.Config(theCardinal) =SetImage(output_BrainPicture_Cfg.Config(theCardinal));
			end			
		end	
	case 'OVERLAY_MISC', 		%Overlay_Misc
		if nargin~=2, error('Usage: output_BrainPicture(''Overlay_Misc'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(output_BrainPicture_Cfg, AFigHandle);
		if theCardinal>0,
			output_BrainPicture_Cfg.Config(theCardinal) =Overlay_Misc(output_BrainPicture_Cfg.Config(theCardinal));
			output_BrainPicture('Repaint', AFigHandle);
		end	
	case 'OPEN_TEMPLATE', 		%Open_Template
		...
		theCardinal =ExistViewer(output_BrainPicture_Cfg, AFigHandle);
		if theCardinal>0,
			output_BrainPicture_Cfg.Config(theCardinal) =Open_Template(output_BrainPicture_Cfg.Config(theCardinal));			
			output_BrainPicture('Repaint', AFigHandle);	
        end	
    case 'CLUSTERSREPORT'
        if nargin~=2, error('Usage: output_BrainPicture(''Clusters Report'', AFigHandle);'); end
       ...
       if theCardinal>0,
          ...
          if ~isempty(find(theConfig.Overlay.VolumeThrd))
                 rp_report( theConfig.Overlay.VolumeThrd,theConfig.Overlay.Header,theConfig.Overlay.ClusterConnectivityCriterion);
          else 
              disp('There is no Cluster !');
          end
       end
	case 'CURRENTTHRD2MASK',	%dong save thrd2mask 2009-09-09
        if nargin~=2, error('Usage: output_BrainPicture(''CurrentThrd2Mask'', AFigHandle);'); end
        AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(output_BrainPicture_Cfg, AFigHandle);
       if theCardinal>0,
			theConfig =output_BrainPicture_Cfg.Config(theCardinal);
			...
			theMask =CurrentThrd2Mask(output_BrainPicture_Cfg.Config(theCardinal));
			if ~isempty(theMask),
				...
				if isequal(filename,0) || isequal(pathname,0),
					...
					rp_writefile(theMask, theMaskFile, size(theConfig.Overlay.Volume), ...
	...
	case 'CURRENTCLUSTER2MASK',			%CurrentCluster2Mask
		if nargin~=2, error('Usage: output_BrainPicture(''CurrentCluster2Mask'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(output_BrainPicture_Cfg, AFigHandle);
		if theCardinal>0,
			theConfig =output_BrainPicture_Cfg.Config(theCardinal);
			...
			theMask =CurrentCluster2Mask(output_BrainPicture_Cfg.Config(theCardinal));
			if ~isempty(theMask),
				...
				if isequal(filename,0) | isequal(pathname,0),
                    ...
                    rp_writefile(theMask~=0, theBinaryMaskFile, size(theConfig.Overlay.Volume), ...
	...
	case 'SAVERECENT',		%SaveRecent		
		if nargin~=2, error('Usage: output_BrainPicture(''SaveRecent'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(output_BrainPicture_Cfg, AFigHandle);
		if theCardinal>0,
			SaveRecent(output_BrainPicture_Cfg.Config(theCardinal), 'RecentOverlay');
			SaveRecent(output_BrainPicture_Cfg.Config(theCardinal), 'RecentUnderlay');
		end
		
	case 'CHANGECOLORELEMENT', 		%ChangeColorElement
		if nargin~=2, error('Usage: output_BrainPicture(''ChangeColorElement'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(output_BrainPicture_Cfg, AFigHandle);
		if theCardinal>0,
			output_BrainPicture_Cfg.Config(theCardinal) =SetColorElements(output_BrainPicture_Cfg.Config(theCardinal));
			output_BrainPicture_Cfg.Config(theCardinal) =SetImage(output_BrainPicture_Cfg.Config(theCardinal));
		end
		
	case 'TOGGLEINFODISPLAY', 		%ToggleInfoDisplay
		if nargin~=2, error('Usage: output_BrainPicture(''ToggleInfoDisplay'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(output_BrainPicture_Cfg, AFigHandle);
		if theCardinal>0,
			ToggleInfoDisplay(output_BrainPicture_Cfg.Config(theCardinal));
		end
	
	case 'UPDATECALLBACK_SAVE2MASK',	%UpdateCallback_Save2Mask
		if nargin~=3, error('Usage: output_BrainPicture(''UpdateCallback_Save2Mask'', AFigHandle, ACallback);'); end
		AFigHandle 	=varargin{1};	
		ACallback   =varargin{2};	
		theCardinal =ExistViewer(output_BrainPicture_Cfg, AFigHandle);
		if theCardinal>0,
			output_BrainPicture_Cfg.Config(theCardinal).Callback.Save2Mask =ACallback;			
		end
	case 'GETSAVEDMASKFILENAME',		%GetSavedMaskFilename
		if nargin~=2, error('Usage: output_BrainPicture(''GetSavedMaskFilename'', AFigHandle);'); end
		AFigHandle 	=varargin{1};			
		theCardinal =ExistViewer(output_BrainPicture_Cfg, AFigHandle);
		if theCardinal>0,
			varargout{1} =output_BrainPicture_Cfg.Config(theCardinal).LastSavedMask;
		else
			varargout{1} ='';
		end
		
	case 'SHOWOVERLAY',	%ShowOverlay
		if nargin>3, error('Usage: output_BrainPicture(''ShowOverlay'', AOverlay); or output_BrainPicture(''ShowOverlay'', AFigHandle, AOverlay);'); end
		AFigHandle 	=varargin{1};
		if ~ischar(AFigHandle),
			AOverlay    =varargin{2};
			theCardinal =ExistViewer(output_BrainPicture_Cfg, AFigHandle);
			if theCardinal>0,
				set(output_BrainPicture_Cfg.Config(theCardinal).hOverlayFile, 'String', AOverlay);
				output_BrainPicture('ChangeOverlay', AFigHandle);
			end
		else	%I will create a new slice-viewer
			AOverlay    =varargin{1};
			theFig = output_BrainPicture; %I can't write "()" considering Matlab 6.5 compatiable
			output_BrainPicture('ShowOverlay', theFig, AOverlay);
			varargout{1} =theFig;
		end
	
	case 'ONKEYPRESS',		%OnKeyPress
		if nargin~=2, error('Usage: output_BrainPicture(''OnKeyPress'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(output_BrainPicture_Cfg, AFigHandle);
		if theCardinal>0,
			output_BrainPicture_Cfg.Config(theCardinal) =OnKeyPress(output_BrainPicture_Cfg.Config(theCardinal));
			%For Updating
			output_BrainPicture_Cfg.Config(theCardinal) =SetImage(output_BrainPicture_Cfg.Config(theCardinal));
			%For Yoke
			output_BrainPicture('SetPosition', AFigHandle);
     ...
     case 'ONLYPOSITIVE',	%dong 2009-09-09
		...
		if theCardinal>0,
            %dong 100327 beging
            if output_BrainPicture_Cfg.Config(theCardinal).Overlay.PNflag==0
                output_BrainPicture_Cfg.Config(theCardinal).Overlay.VolumeForFlag=output_BrainPicture_Cfg.Config(theCardinal).Overlay.VolumeThrd;
                output_BrainPicture_Cfg.Config(theCardinal).Overlay.PNflag=1;
            end
			output_BrainPicture_Cfg.Config(theCardinal).Overlay.VolumeThrd(output_BrainPicture_Cfg.Config(theCardinal).Overlay.VolumeForFlag<0)=0;
            output_BrainPicture_Cfg.Config(theCardinal).Overlay.VolumeThrd(output_BrainPicture_Cfg.Config(theCardinal).Overlay.VolumeForFlag>output_BrainPicture_Cfg.Config(theCardinal).Overlay.ValueThrdAbsolute)=output_BrainPicture_Cfg.Config(theCardinal).Overlay.VolumeForFlag(output_BrainPicture_Cfg.Config(theCardinal).Overlay.VolumeForFlag>output_BrainPicture_Cfg.Config(theCardinal).Overlay.ValueThrdAbsolute);
            %dong 100327 end
			output_BrainPicture_Cfg.Config(theCardinal) =SetImage(output_BrainPicture_Cfg.Config(theCardinal));			
        end	
...
function Result =InitControls(AFilename, ACallback)	
	...
	set(theFig, 'DeleteFcn', sprintf('output_BrainPicture(''Delete'', %g);', theFig)  );
	set(theFig, 'KeyPressFcn', sprintf('output_BrainPicture(''OnKeyPress'', %g);', theFig)  );
	...
	hCrosshair =uicontrol(theFig, 'Style','checkbox', 'Visible','off','Units','pixels', ...
					...
					'Callback', sprintf('output_BrainPicture(''Repaint'', %g);',theFig) , ...
					'BackgroundColor', get(theFig,'Color'), ...
					'Position',[theLeft, theBottom+75, 70,15]);
	
	
	theEditCallbackFcn =sprintf('output_BrainPicture(''SetPosition'', %g);', theFig);
	...
	hMagnify =uicontrol(theFig, 'Style','popupmenu', 'Visible','off','Units','pixels', ...
						...
						'BackgroundColor', get(theFig,'Color'), ...
						'Enable', 'off', ...
						'Callback', sprintf('output_BrainPicture(''Magnify'', %g);',theFig), ...
	...
	hMniTal =uicontrol(theFig, 'Style','popupmenu', 'Visible','off','Units','pixels', ...
						...
						'BackgroundColor', get(theFig,'Color'), ...
						'Enable', 'on', ...
						'Callback', sprintf('output_BrainPicture(''MNI/Talairach'', %g);',theFig), ...
	...
	hMsgLabel =uicontrol(theFig, 'Style','text', 'Visible','off','Units','pixels', ...
			  ...
			  'ButtonDownFcn', sprintf('output_BrainPicture(''ToggleInfoDisplay'', %g);',theFig), ...
	...
	theIcon =imread(fullfile(rp_misc( 'WhereIsREST'), 'icoTransverse.jpg'));
	hViewTransverse =uicontrol(theFig, 'Style','pushbutton',  ...
							  'Visible','off','Units','pixels', ...
							  'Callback', sprintf('output_BrainPicture(''TransverseView'', %g);',theFig) , ...
							  'CData', theIcon,...
							  'Position',[theLeft, theBottom, 34,36]);
	theLeft =OffsetX+MarginX+30; theBottom =OffsetY +MarginY-2;
	theIcon =imread(fullfile(rp_misc( 'WhereIsREST'), 'icoSagittal.jpg'));
	hViewSagittal =uicontrol(theFig, 'Style','pushbutton',  ...
							  'Visible','off','Units','pixels', ...
							  'Callback', sprintf('output_BrainPicture(''SagittalView'', %g);',theFig) , ...
							  'CData', theIcon,...
							  'Position',[theLeft, theBottom, 34,36]);
	theLeft =OffsetX+MarginX+34+30; theBottom =OffsetY +MarginY-2;
	theIcon =imread(fullfile(rp_misc( 'WhereIsREST'), 'icoCoronal.jpg'));
	hViewCoronal =uicontrol(theFig, 'Style','pushbutton',  ...
							  'Visible','off','Units','pixels', ...
							  'Callback', sprintf('output_BrainPicture(''CoronalView'', %g);',theFig) , ...
							  'CData', theIcon,...
							  'Position',[theLeft, theBottom, 34,36]);
	theLeft =OffsetX+MarginX+34+34+30; theBottom =OffsetY +MarginY-2;
	theIcon =imread(fullfile(rp_misc( 'WhereIsREST'), 'icoOrthogonal.jpg'));
	hViewOrthogonal =uicontrol(theFig, 'Style','pushbutton',  ...
							  'Visible','off','Units','pixels', ...
							  'Callback', sprintf('output_BrainPicture(''OrthogonalView'', %g);',theFig) , ...
							  'CData', theIcon,...
							  'Position',[theLeft, theBottom, 34,36]);
	theLeft =OffsetX+MarginX+34+30; theBottom =OffsetY +MarginY+34;	
	hViewMontage =uicontrol(theFig, 'Style','pushbutton',  ...
							  'Visible','off','Units','pixels', ...
							  'Callback', sprintf('output_BrainPicture(''Montage'', %g);',theFig) , ...
	...
	hUnderlayRecent =uicontrol(theFig, 'Style','popupmenu', 'Visible','off','Units','pixels', ...
			  ...
			  'Callback', sprintf('output_BrainPicture(''ClickRecentUnderlay'', %g);',theFig), ...
			  'BackgroundColor', get(theFig,'Color'), ...			  
	...
	hUnderlayFile =uicontrol(theFig, 'Style','edit', 'Visible','off','Units','pixels', ...
							  'String', '', ...
							  'BackgroundColor', 'white', ...
							  'Callback', sprintf('output_BrainPicture(''ChangeUnderlay'', %g);',theFig), ...
							  'Position',[theLeft, theBottom, 150,20]);
	theLeft =OffsetX+116; theBottom =MarginY+225;
	uicontrol(theFig, 'Style','pushbutton', 'Visible','off','Units','pixels', ...
		  'Callback', sprintf('output_BrainPicture(''UnderlaySelection'', %g);',theFig) , ...
	...
	hOverlayRecent =uicontrol(theFig, 'Style','popupmenu', 'Visible','off','Units','pixels', ...
			  ...
			  'Callback', sprintf('output_BrainPicture(''ClickRecentOverlay'', %g);',theFig), ...
			  'BackgroundColor', get(theFig,'Color'), ...			  
			  'Position',[theLeft, theBottom, 75,20]);	 
	...
	hOverlayFile =uicontrol(theFig, 'Style','edit', 'Visible','off','Units','pixels', ...
					  'String', '', ...					  
					  'BackgroundColor', 'white', ...
					  'Callback', sprintf('output_BrainPicture(''ChangeOverlay'', %g);',theFig), ...
					  'Position',[theLeft, theBottom, 150,20]);
	theLeft =OffsetX+116; theBottom =MarginY+185;
	uicontrol(theFig, 'Style','pushbutton', 'Visible','off','Units','pixels', ...
		  'Callback', sprintf('output_BrainPicture(''OverlaySelection'', %g);',theFig) , ...
		  'String', '...','FontWeight', 'bold', ...
		  'Position',[theLeft, theBottom, 34,15]); 
	...
	hEdtThrdValue =uicontrol(theFig, 'Style','edit', 'Visible','off','Units','pixels', ...
					'String', '', ...					
					'Callback', sprintf('output_BrainPicture(''Overlay_SetThrdAbsValue'', %g);',theFig), ...	
					 'BackgroundColor', 'white', ...
   ...
   hEdtPValue =uicontrol(theFig, 'Style','edit', 'Visible','off','Units','pixels', ...%P-value need improve dong
    ...
    hdf =uicontrol(theFig, 'Style','pushbutton',  ... %add df dong 090921
							  'Visible','off','Units','pixels', ...
							  'Callback', sprintf('output_BrainPicture(''DF'', %g);',theFig),...	
							  'String', 'df', ...
...
uicontrol(theFig, 'Style','text', 'Visible','off','Units','pixels', ...%dong
     ...
     uicontrol(theFig, 'Style','pushbutton', 'Visible','off','Units','pixels', ...  %only + dong 090921
			'String', '+', ...
			'Callback', sprintf('output_BrainPicture(''OnlyPositive'', %g);',theFig),...			
			'Position',[theLeft+112, theBottom+80, 15,18]);
     uicontrol(theFig, 'Style','pushbutton', 'Visible','off','Units','pixels', ...  %only + dong 090921
			'String', '-', ...
			'Callback', sprintf('output_BrainPicture(''OnlyNegative'', %g);',theFig),...			
			'Position',[theLeft+129, theBottom+80, 15,18]);
        
	uicontrol(theFig, 'Style','pushbutton', 'Visible','off','Units','pixels', ...
			'String', 'Cluster Size', ...
			'Callback', sprintf('output_BrainPicture(''Overlay_SetThrdClusterSize'', %g);',theFig),...		%here dong	
	...
	hOverlayMisc =uicontrol(theFig, 'Style','popupmenu', 'Visible','off','Units','pixels', ...
					...
					'BackgroundColor', get(theFig,'Color'), ...
					'Enable', 'on', ...
					'Callback', sprintf('output_BrainPicture(''Overlay_Misc'', %g);',theFig), ...
	...
	hTemplate =uicontrol(theFig, 'Style','popupmenu', 'Visible','off','Units','pixels', ...
					...
					'BackgroundColor', get(theFig,'Color'), ...
					'Enable', 'on', ...
					'Callback', sprintf('output_BrainPicture(''Open_Template'', %g);',theFig), ...
					'Position',[theLeft+75, theBottom+55, 70,24]);
	uicontrol(theFig, 'Style','pushbutton', 'Visible','off','Units','pixels', ...
			'String', 'Save Cluster', ...
			'Callback', sprintf('output_BrainPicture(''CurrentCluster2Mask'', %g);',theFig),...			
   ...
   uicontrol(theFig, 'Style','pushbutton', 'Visible','off','Units','pixels', ...%dong 090920
			...
			'Callback', sprintf('output_BrainPicture(''CurrentThrd2Mask'', %g);',theFig),...			
			'Position',[theLeft+5, theBottom+5, 70,24]);
    uicontrol(theFig, 'Style','pushbutton', 'Visible','off','Units','pixels', ...%dong 090920
			'String', 'Cl. Report', ...
           'Callback', sprintf('output_BrainPicture(''ClustersReport'', %g);',theFig),...				
			'Position',[theLeft+75, theBottom+5, 70,24]);
	% uicontrol(theFig, 'Style','pushbutton', 'Units','pixels', ...
	%		 'String', 'Thrd2Mask', ...
	%		 'Callback', sprintf('output_BrainPicture(''Overlay_SetThrdClusterSize'', %g);',theFig),...			
	%		 'Position',[theLeft+75, theBottom+5, 70,24]);
			
	%Colorbar for overlay, do as AFNI, 20070921
	theAxesButtonDownFcn =sprintf('output_BrainPicture(''ChangeColorElement'', %g);',theFig);
...
function Result =DeleteFigure(AGlobalConfig, AFigHandle)
	...
	if x>0,			
		...
		isExistFig =rp_misc( 'ForceCheckExistFigure' , AGlobalConfig.Config(x).hFig);
		if isExistFig,
			%Save the recent menu
			output_BrainPicture('SaveRecent', AFigHandle);
...
function Result =SetImage(AConfig)
	...
	set(AConfig.hFig, 'Name', sprintf('Slice Viewer -- For Check use')); %YAN Chao-Gan, 090919.
...
function Result =SetView_Orthogonal(AConfig)	
	...
	if license('test','image_toolbox')==1 && theMagnifyCoefficient~=1, 
        if rp_misc('GetMatlabVersion')>=7.4   %YAN Chao-Gan 090401: The imresize function has been completely rewritten in Matlab R2007a. Fixed the bug of 'Set Overlay's Color bar' in Matlab R2007a or latter version.
...
function SetMessage(AConfig)
	...
	if SeeOverlay(AConfig),
		...
		if all(theOverlayPos<=size(AConfig.Overlay.Volume)) ...
			...
			output_BrainPicture('SetPhysicalPosition', AConfig.hFig, [0 0 0]);
			output_BrainPicture('Repaint', AConfig.hFig);
			return;
		end
...
function Result =UpdateCallback(AConfig, ACallback, ACallbackCaption)
	...
	% else
		...
		% if ~isempty(ACallback) && (ischar(ACallback) || isa(ACallback, 'function_handle')),
			% theCallback =sprintf('output_BrainPicture(''RunCallback'', ''%s'');',AConfig.Filename);
...
function Transforming_MNI_Talairach(AConfig)
	...
	case 2, %'From Talairach to MNI'
		...
		thePosition =round(rp_tal2mni(AConfig.LastPosition -AConfig.Origin) +AConfig.Origin);
		AConfig =UpdatePosition(AConfig, thePosition);
		isNeedUpdate =true;
	case 3, %'From MNI to Talairach'
		% AConfig.LastPosition 
		thePosition=round(rp_mni2tal(AConfig.LastPosition -AConfig.Origin) +AConfig.Origin);
		AConfig =UpdatePosition(AConfig, thePosition);
		isNeedUpdate =true;
	otherwise
	end
	...
	if isNeedUpdate,
		ShowPositionInEdit(AConfig);
		SetMessage(AConfig);
		output_BrainPicture('SetPosition', AConfig.Filename);
	end
...
%Input: Need AConfig.Filename specified	
	try
		[theVolume,theVoxelSize, Header] =rp_readfile(AConfig.Filename);  %%Yan 080610
	...
	theAxesButtonDownFcn =sprintf('output_BrainPicture(''ClickPosition'', %g);', AConfig.hFig);
...
%Threshold for Cluster Size or calculated cluster size from cluster-Raidus %This function must be called after thresholding the Value already!		
	...
	if AConfig.Overlay.ClusterRadiusThrd~=0, %Raidus(mm) 
        ...
        maskROI =rp_SphereROI( 'BallDefinition2Mask' , sprintf('ROI Center(mm)=(0, 0, 0); Radius=%g mm.', AConfig.Overlay.ClusterRadiusThrd), size(AConfig.Overlay.Volume), AConfig.Overlay.VoxelSize, AConfig.Overlay.Header);  %%Yan 080610 and 081223
...
%Such as AAL, BRODMANN ...	
	...
	[pathstr, name, ext] = fileparts(AConfig.Overlay.Filename);
	if strcmpi(name, 'aal'),		
		AColorMap =rp_ReadLutColorScheme(fullfile(rp_misc( 'WhereIsREST'),'Template', 'aal.nii.lut'));	%YAN Chao-Gan 081223: use the NIFTI image information from the MRIcroN	 
	...
	elseif strcmpi(name, 'brodmann'),		
		AColorMap =rp_ReadLutColorScheme(fullfile(rp_misc( 'WhereIsREST'),'Template', 'brodmann.nii.lut'));		%YAN Chao-Gan 081223: use the NIFTI image information from the MRIcroN	  
		theBlackPos=sum(AColorMap, 2)==0;
		AColorMap(theBlackPos, :) =1; %Black to White for I use black always as the background
		Result =overlay_ind2rgb(1+floor(Result), AColorMap);%Overpass the first color which may be backgound color	
	end
...
function Result =LoadOverlay(AConfig, AFilename)
	...
	Result =AConfig;
	try
		[theVolume,theVoxelSize, Header] =rp_readfile(AFilename); %%Yan 080610
        theOrigin=Header.Origin; %%Yan 080610
	...
	if strcmpi(name, 'aal'),		
		theInfoTxt =fullfile(rp_misc( 'WhereIsREST'),'Template', 'aal.nii.txt'); %YAN Chao-Gan 081223: use the NIFTI image information from the MRIcroN	 
...
%                 },'Correction Thresholds by AlphaSim');
            rp_CorrectionThresholdsByAlphaSim_gui;     %Revised by YAN Chao-Gan 091108: add situation in rmm=5 and rmm=6
        case 8 % DONG FDR 100118
            [AConfig.Overlay.Qvalue,AConfig.Overlay.Qmaskname,AConfig.Overlay.Conproc,AConfig.Overlay.Tchoose]=rp_FDR_gui(AConfig.Overlay.Qvalue,AConfig.Overlay.Qmaskname,AConfig.Overlay.Conproc,AConfig.Overlay.Tchoose);
            ...
            if ~isempty(AConfig.Overlay.Qmaskname)
                [MaskData MaskVox MaskHead]=rp_readfile(AConfig.Overlay.Qmaskname);
                [pID,pN] = rp_FDR(ValueP(find(MaskData)),AConfig.Overlay.Qvalue);
            else
                [pID,pN] = rp_FDR(ValueP,AConfig.Overlay.Qvalue);
            end
...
function Result =Open_Template(AConfig)	
	...
	case 2, %'Open Template AAL'
		rp_misc( 'CheckTemplate');
		theNewOverlay =fullfile(rp_misc( 'WhereIsREST'), 'Template', 'aal.nii'); %YAN Chao-Gan 081223: use the NIFTI image information from the MRIcroN	 
		set(AConfig.hOverlayFile, 'String', theNewOverlay);
		% output_BrainPicture('ChangeOverlay', AConfig.hFig);
		AConfig =LoadOverlay(AConfig, theNewOverlay);
		set(AConfig.hSeeOverlay, 'Value', 1);
	case 3, %'Open Template Brodmann'
		rp_misc( 'CheckTemplate');
		theNewOverlay =fullfile(rp_misc( 'WhereIsREST'), 'Template', 'brodmann.nii'); %YAN Chao-Gan 081223: use the NIFTI image information from the MRIcroN	 
		set(AConfig.hOverlayFile, 'String', theNewOverlay);
		% output_BrainPicture('ChangeOverlay', AConfig.hFig);			
		AConfig =LoadOverlay(AConfig, theNewOverlay);
		set(AConfig.hSeeOverlay, 'Value', 1);
	case 4, %'Open Template Ch2'
		rp_misc( 'CheckTemplate');
		theNewUnderlay =fullfile(rp_misc( 'WhereIsREST'), 'Template', 'ch2.nii'); %YAN Chao-Gan 100403: use the ch2 bet image information from the MRIcroN	 
	...
	case 5, %'Open Template Ch2'
		rp_misc( 'CheckTemplate');
		theNewUnderlay =fullfile(rp_misc( 'WhereIsREST'), 'Template', 'ch2bet.nii'); %YAN Chao-Gan 100403: use the ch2 bet image information from the MRIcroN	 
		set(AConfig.hUnderlayFile, 'String', theNewUnderlay);
		% output_BrainPicture('ChangeOverlay', AConfig.hFig);	
		AConfig.Filename =theNewUnderlay;
		AConfig =InitUnderlay(AConfig);			
...
function Result =InitRecent(AConfig)
	Result =AConfig;	 
	theRecentCfg =fullfile(tempdir,['RecentUnderlay','_',rp_misc('GetCurrentUser'),'.txt']); %YAN Chao-Gan, 100420.  %theRecentCfg =fullfile(rp_misc( 'WhereIsREST'), 'RecentUnderlay.txt');
	if  exist(theRecentCfg, 'file')==2,
		[Result.Recent.Underlay] =textread(theRecentCfg,'%s', 'delimiter','\n');
	else
		Result.Recent.Underlay={};
    end
	theRecentCfg =fullfile(tempdir,['RecentOverlay','_',rp_misc('GetCurrentUser'),'.txt']);  %YAN Chao-Gan, 100420. %theRecentCfg =fullfile(rp_misc( 'WhereIsREST'), 'RecentOverlay.txt');
	if  exist(theRecentCfg, 'file')==2,
...
%AType = RecentUnderlay.txt or RecentOverlay.txt
	theRecentCfg =fullfile(tempdir,[AType,'_',rp_misc('GetCurrentUser'),'.txt']);  % YAN Chao-Gan, 100420. %theRecentCfg =fullfile(rp_misc( 'WhereIsREST'), [AType, '.txt']);
...
function rp_report(data,head,ClusterConnectivityCriterion)
