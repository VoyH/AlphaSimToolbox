function fout = rp_progress(x,whichbar, varargin)
%Show a progress bar for REST by Xiao-Wei Song
%Revise from Matlab 2006b waitbar, at least 2 parameters
%Usage:
%rp_progress(APercent,hChild);
%rp_progress(APercent,hChild, AMessage, 'Name', ATitle);
%rp_progress(APercent,hParent,sprintf('Total progress:\n%s',AMessage), 'Name', ATitle);
%rp_progress(APercent,hChild, theMsg, 'Name', ATitle);	
%------------------------------------------------------------------------------------------------------------------------------
...
elseif nargin==2    
	f=whichbar;
	if ~isnumeric(f),
		error('rp_progress, Input arguments not valid.');
	end
