function fg=spm_image_reorient(op,varargin)
% image and header display
% FORMAT spm_image_reorient
%_______________________________________________________________________
%
% spm_image_reorient is an interactive facility that allows orthogonal sections
...
% John Ashburner
% $Id: spm_image_reorient.m 3691 2010-01-20 17:08:30Z guillaume $


global st
global spm_image_reorient_Parameters %YAN Chao-Gan 101010
...
if nargin == 0,
    ...
    spm_image_reorient('init',P);
    return;
end;

if isstruct(op)
    % job data structure
    spm_image_reorient('init', op.data{1});
    return;
end;
...
if strcmp(op,'repos'),
    ...
    % spm_orthviews('MaxBB');
    spm_image_reorient('zoom_in');
    spm_image_reorient('update_info');
...
if strcmp(op,'addblobs'),
    ...
    for i=1:nblobs,
        ...
        set(st.blobber,'String','Remove Blobs','Callback','spm_image_reorient(''rmblobs'');');
    end;
...
if strcmp(op,'rmblobs'),
    ...
    set(st.blobber,'String','Add Blobs','Callback','spm_image_reorient(''addblobs'');');
...
if strcmp(op,'reorient'),
    ...
    mat = spm_matrix(st.B);
    spm_image_reorient_Parameters.ReorientMat = mat;
    %YAN Chao-Gan 101010
%     if det(mat)<=0
%         spm('alert!','This will flip the images',mfilename,0,1);
%     end;
    P = spm_image_reorient_Parameters.ReorientFileList;
...
%         spm_image_reorient('init',st.vols{1}.fname);
%     end;
    close(fg);
    return;
end;
...
if strcmp(op,'DefineROI'),
    ...
    spm_image_reorient_Parameters.pos=posTemp;
    close(fg);
    return;
end;
...
if ~isfield(spm_image_reorient_Parameters,'ReorientFileList')
    spm_image_reorient_Parameters.ReorientFileList={P};
end
...
spm_orthviews('MaxBB');
st.callback = 'spm_image_reorient(''shopos'');';
...
uicontrol(fg,'Style','Frame','Position',[60 25 200 325].*WS,'DeleteFcn','spm_image_reorient(''reset'');');
...
uicontrol(fg,'Style','edit','Callback','spm_image_reorient(''repos'',1);','Position',[175 220+moveup_num 065 020].*WS,'String','0','ToolTipString','translate');
uicontrol(fg,'Style','edit','Callback','spm_image_reorient(''repos'',2);','Position',[175 200+moveup_num 065 020].*WS,'String','0','ToolTipString','translate');
uicontrol(fg,'Style','edit','Callback','spm_image_reorient(''repos'',3);','Position',[175 180+moveup_num 065 020].*WS,'String','0','ToolTipString','translate');
uicontrol(fg,'Style','edit','Callback','spm_image_reorient(''repos'',4);','Position',[175 160+moveup_num 065 020].*WS,'String','0','ToolTipString','rotate');
uicontrol(fg,'Style','edit','Callback','spm_image_reorient(''repos'',5);','Position',[175 140+moveup_num 065 020].*WS,'String','0','ToolTipString','rotate');
uicontrol(fg,'Style','edit','Callback','spm_image_reorient(''repos'',6);','Position',[175 120+moveup_num 065 020].*WS,'String','0','ToolTipString','rotate');
% uicontrol(fg,'Style','edit','Callback','spm_image_reorient(''repos'',7);','Position',[175 100 065 020].*WS,'String','1','ToolTipString','zoom');
% uicontrol(fg,'Style','edit','Callback','spm_image_reorient(''repos'',8);','Position',[175  80 065 020].*WS,'String','1','ToolTipString','zoom');
% uicontrol(fg,'Style','edit','Callback','spm_image_reorient(''repos'',9);','Position',[175  60 065 020].*WS,'String','1','ToolTipString','zoom');

uicontrol(fg,'Style','Pushbutton','String','Reorient now','fontsize',15,'Callback','spm_image_reorient(''reorient'');',...
         'Position',[330 50 130 50].*WS,'ToolTipString','modify position information of the images');
% uicontrol(fg,'Style','Pushbutton','String','Reorient images','Callback','spm_image_reorient(''reorient'')',...
%          'Position',[70 35 125 020].*WS,'ToolTipString','modify position information of selected images');
     
uicontrol(fg,'Style','Pushbutton','visible','off','String','Define ROI','Callback','spm_image_reorient(''DefineROI'');',...
         'Position',[175 35 80 020].*WS,'ToolTipString','reset orientations of selected images');
% uicontrol(fg,'Style','Pushbutton','String','Reset...','Callback','spm_image_reorient(''resetorient'')',...
...
uicontrol(fg,'Style','Text','visible','on', 'Position',[75 320-movedown_num 170 016].*WS,'String','New origin position');
...
st.mp = uicontrol(fg,'Style','edit','visible','on', 'Position',[110 295-movedown_num 135 020].*WS,'String','','Callback','spm_image_reorient(''setposmm'');','ToolTipString','move crosshairs to mm coordinates');
st.vp = uicontrol(fg,'Style','edit','visible','on', 'Position',[110 275-movedown_num 135 020].*WS,'String','','Callback','spm_image_reorient(''setposvx'');','ToolTipString','move crosshairs to voxel coordinates');
...
st.zoomer = uicontrol(fg,'Style','popupmenu' ,'visible','off','Position',[315 75 125 20].*WS,...
    ...
    'Callback','spm_image_reorient(''zoom_in'')','ToolTipString','zoom in by different amounts');
c = 'if get(gco,''Value'')==1, spm_orthviews(''Space''), else, spm_orthviews(''Space'', 1);end;spm_image_reorient(''zoom_in'')';
...
st.win = uicontrol(fg,'Style','popupmenu','visible','off','Position',[315 35 125 20].*WS,...
    'String',char('Auto Window','Manual Window'),'Callback','spm_image_reorient(''window'');','ToolTipString','range of voxel intensities displayed');
% uicontrol(fg,'Style','pushbutton','Position',[315 35 125 20].*WS,...
%   'String','Window','Callback','spm_image_reorient(''window'');','ToolTipString','range of voxel intensities % displayed');
st.blobber = uicontrol(fg,'Style','pushbutton','visible','off','Position',[450 35 125 20].*WS,...
    'String','Add Blobs','Callback','spm_image_reorient(''addblobs'');','ToolTipString','superimpose activations');
set(fg,'Menubar','none');
