...
% getting the motion correction results. Assuming it is located in the same folder as the images
 ...
 if st~=size(moves,1)
     printf('Something wrong happened during motion correction.\n');
     printf('Number of images selected here: %d is different from \n', st);
     printf('    the number of images: %d in motion correction. \n', size(moves, 1));
     return;
 end
 dat(isnan(dat))=0;
