function varargout = rp_DicomSorter_gui(varargin)
%   varargout = rp_DicomSorter_gui(varargin)
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @rp_DicomSorter_gui_OpeningFcn, ...
                   'gui_OutputFcn',  @rp_DicomSorter_gui_OutputFcn, ...
                   'gui_LayoutFcn',  [], ...
                   'gui_Callback',   []);
...
% --- Executes just before rp_DicomSorter_gui is made visible.
function rp_DicomSorter_gui_OpeningFcn(hObject, eventdata, handles, varargin)
...
movegui(handles.rp_DicomSorter_gui, 'center');
guidata(hObject, handles);

% --- Outputs from this function are returned to the command line.
function varargout = rp_DicomSorter_gui_OutputFcn(hObject, eventdata, handles)
...
if ~isdir(handles.Cfg.OutputDir) % jia xi-ze in 150711
...
for i=1:size(handles.Cfg.DataDirs, 1)
    ...
    dirlist={};
    dirlist=[rp_DicomSorter(handles.Cfg.DataDirs{i},outfile,dicomtype,handles.Cfg.DirectoryHierarchy);dirlist];
    if Anonymizeflag
        ...
        for j=1:size(dirlist,1)
            ...
            rp_ChangeDicomInfo(dirlist{j},outfileA,'',dicomtype);
        end
    end
end
rp_waitbar;
...
function InitControlProperties(hObject, handles)
	...
	uimenu(handles.hContextMenu, 'Label', 'Remove selected directory', 'Callback', 'rp_DicomSorter_gui(''DeleteSelectedDataDir'',gcbo,[], guidata(gcbo))');
	uimenu(handles.hContextMenu, 'Label', 'Add recursively all sub-folders of a directory', 'Callback', 'rp_DicomSorter_gui(''RecursiveAddDataDir'',gcbo,[], guidata(gcbo))');
	uimenu(handles.hContextMenu, 'Label', '=============================');	
	uimenu(handles.hContextMenu, 'Label', 'Remove all data directories', 'Callback', 'rp_DicomSorter_gui(''ClearDataDirectories'',gcbo,[], guidata(gcbo))');
...
function RecursiveAddDataDir(hObject, eventdata, handles)
	...
	if ischar(theDir),
		...
		try
			rp_RecursiveDir(theDir, 'rp_DicomSorter_gui(''SetDataDir'',gcbo, ''%s'', guidata(gcbo) )');
		catch
