function varargout = rp_ExtractROITC_gui(varargin)
%   varargout = rp_RegressOutCovariates_gui(varargin)
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @rp_ExtractROITC_gui_OpeningFcn, ...
                   'gui_OutputFcn',  @rp_ExtractROITC_gui_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before rp_ExtractROITC_gui is made visible.
function rp_ExtractROITC_gui_OpeningFcn(hObject, eventdata, handles, varargin)
...
movegui(handles.rp_ExtractROITC_gui, 'center');
guidata(hObject, handles);



% --- Outputs from this function are returned to the command line.
function varargout = rp_ExtractROITC_gui_OutputFcn(hObject, eventdata, handles) 
...
for i=1:tmpSize
    ...
    if MutiValueFlag==1 %110504 DONG Multi label values in a single mask file
        rp_ExtractROITC_MultiValues(handles.Cfg.DataDirs{i,1}, handles.Cfg.ROIDef,handles.Cfg.OutputDir);
    else
        rp_ExtractROITC(handles.Cfg.DataDirs{i,1}, handles.Cfg.ROIDef,handles.Cfg.OutputDir);
    end
end
rp_waitbar;
...
function InitControlProperties(hObject, handles)
	...
	uimenu(handles.hContextMenu, 'Label', 'Remove selected directory', 'Callback', 'rp_ExtractROITC_gui(''DeleteSelectedDataDir'',gcbo,[], guidata(gcbo))');
	uimenu(handles.hContextMenu, 'Label', 'Add recursively all sub-folders of a directory', 'Callback', 'rp_ExtractROITC_gui(''RecursiveAddDataDir'',gcbo,[], guidata(gcbo))');
	uimenu(handles.hContextMenu, 'Label', '=============================');	
	uimenu(handles.hContextMenu, 'Label', 'Remove all data directories', 'Callback', 'rp_ExtractROITC_gui(''ClearDataDirectories'',gcbo,[], guidata(gcbo))');
...
function RecursiveAddDataDir(hObject, eventdata, handles)
	...
	if ischar(theDir),
		...
		try
			rp_RecursiveDir(theDir, 'rp_ExtractROITC_gui(''SetDataDir'',gcbo, ''%s'', guidata(gcbo) )');
		catch
