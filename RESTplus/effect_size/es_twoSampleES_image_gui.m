function varargout = es_twoSampleES_image_gui(varargin)
%   varargout = es_twoSampleES_image_gui(varargin)
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @es_twoSampleES_image_gui_OpeningFcn, ...
                   'gui_OutputFcn',  @es_twoSampleES_image_gui_OutputFcn, ...
                   'gui_LayoutFcn',  [], ...
                   'gui_Callback',   []);
...
function varargout = es_twoSampleES_image_gui_OutputFcn(hObject, eventdata, handles)
...
% 151118 modify by Jia Xi-Ze
ES_OutputName=[handles.Cfg.OutputDir filesep 'estbx_TwoSample_EffectSize' outname];
TT_OutputName=[handles.Cfg.OutputDir filesep 'estbx_TwoSample_T_' outname];

es_twoSampleES_image(DependentDirs,ES_OutputName,MaskFile);
% es_twoSampleTT_image(DependentDirs,TT_OutputName,...
%     MaskFile,CovariateDirs,OtherCovariates);  %By jiaxize 160113
...
uimenu(handles.hContextMenu, 'Label', 'Remove selected group', 'Callback', 'es_twoSampleES_image_gui(''DeleteSelectedG'',gcbo,[], guidata(gcbo))');
uimenu(handles.hContextMenu, 'Label', '=============================');
uimenu(handles.hContextMenu, 'Label', 'Remove all data groups', 'Callback', 'es_twoSampleES_image_gui(''ClearG'',gcbo,[], guidata(gcbo))');
...
% handles    structure with handles and user data (see GUIDATA)
    outname = get(handles.edtOutput,'String');
    [a,name,c]=fileparts(outname);
    name = strcat('estbx_TwoSample_T',name);
    theESMapDir = [handles.Cfg.OutputDir filesep [a,name,c]];
...
% --- Executes during object creation, after setting all properties.
function SeeResults_CreateFcn(hObject, eventdata, handles)
% hObject    handle to SeeResults (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
function figTTestandES_ButtonDownFcn(hObject, eventdata, handles)
% hObject    handle to figTTestandES (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
