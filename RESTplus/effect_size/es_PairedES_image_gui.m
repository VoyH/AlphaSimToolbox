function varargout = es_PairedES_image_gui (varargin)
%   varargout = effect_size_gui(varargin)
%   Effect Size GUI for Paired t-test.
%   By Gao Zhong-Zhan and Li Xun.
%   Center for Cognition and Brain Disorders, Hangzhou Normal University, 310015
%	http://www.restfmri.net
% 	Mail to Authors:  gaozhongzhan@126.com, lixun2011@126.com
%	Version=1.0;
%	Release=20140814;
%----------------------------------------------------   ----------------------
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @es_PairedES_image_gui_OpeningFcn, ...
                   'gui_OutputFcn',  @es_PairedES_image_gui_OutputFcn, ...
                   'gui_LayoutFcn',  [], ...
                   'gui_Callback',   []);
...
function es_PairedES_image_gui_OpeningFcn(hObject, eventdata, handles, varargin)
handles.Cfg.GroupDirs ={};
handles.Cfg.Base=0;
handles.Cfg.MaskFile = '';
handles.Cfg.OutputDir=pwd;
handles.Cfg.OutputName='_C1-C2';
...
function varargout = es_PairedES_image_gui_OutputFcn(hObject, eventdata, handles)
...
if lenC(1) >=2
    msgbox('Only two conditions are allowed in effect size.','Warning');
else
...
%     drawnow;
es_PairedES_image(DependentDirs, OutputName,MaskFile);
% es_PairedTT_image(DependentDirs,OutputName,MaskFile);
...
msgbox({ 'The Effect size (ES) is performed between the two paired groups. Please make sure the correspondence of the images between the two paired groups.';...
    'The value in each voxel in the output image is a Cohen''s d value (positive value means that group 1 is greater than group 2, negative one means otherwise).';...
    'Interpretation to the ES value is that, small ES: Cohen''s d = 0.2 ~ 0.5; medium: d = 0.5 ~ 0.8; large: d = 0.8 and higher.';...
    'For negative case, small ES: Cohen''s d = -0.5 ~ -0.2; medium: d = -0.8 ~ -0.5; large: d = -0.8 and lower. ';...
    },'Help');
...
uimenu(handles.hContextMenu, 'Label', 'Remove selected group', 'Callback', 'es_PairedES_image_gui(''DeleteSelectedG'',gcbo,[], guidata(gcbo))');
uimenu(handles.hContextMenu, 'Label', '=============================');
uimenu(handles.hContextMenu, 'Label', 'Remove all data groups', 'Callback', 'es_PairedES_image_gui(''ClearG'',gcbo,[], guidata(gcbo))');
...
function figTTest1_ButtonDownFcn(hObject, eventdata, handles)
% hObject    handle to figTTest1 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)


% --- Executes on button press in seeresult.
function seeresult_Callback(hObject, eventdata, handles)
% hObject    handle to seeresult (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
    ...
    theESMapDir = [handles.Cfg.OutputDir filesep [a,b,c]];
    theESMapDir = strcat(theESMapDir,'.nii');
    currentford = which('NIStatES');
    currentford = fileparts(currentford);
    currentford = [currentford filesep 'Template' filesep 'ch2.nii'];
    
    underlaydir = currentford;
    theviwer = es_sliceviewer('SHOWUNDERLAY',underlaydir);
    es_sliceviewer('SHOWOVERLAY',theviwer,theESMapDir);

% --- Executes during object creation, after setting all properties.
function seeresult_CreateFcn(hObject, eventdata, handles)
% hObject    handle to seeresult (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
% --- Executes during object deletion, before destroying properties.

function seeResults_DeleteFcn(hObject, eventdata, handles)
...
% --- Executes during object deletion, before destroying properties.
function seeresult_DeleteFcn(hObject, eventdata, handles)
% hObject    handle to seeresult (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
