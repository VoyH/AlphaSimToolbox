function [Data_Corrected, ClusterSize, Header]=rp_GRF_Threshold(StatsImgFile,VoxelPThreshold,IsTwoTailed,ClusterPThreshold,OutputName,MaskFile,Flag,Df1,Df2,VoxelSize,Header, dLh)
...
%                       - 'DO NOT OUTPUT IMAGE': means called by rp_sliceviewer, and do not need to write into a file.
...
%     VoxelSize         - The Voxel's size of the image inputed. Defined when call by rp_sliceviewer.
%     Header            - The Header of the nifti image.
%     dLh               - Smoothness estimated as sqrt(det(Lambda)) with rp_Smoothest, will be used in inference. 
...
if (~exist('Flag','var')) || (exist('Flag','var') && isempty(Flag))
    ...
    if ischar(StatsImgFile)
        [Data VoxelSize Header]=rp_readfile(StatsImgFile);
    end
...
if (~exist('dLh','var')) || (exist('dLh','var') && isempty(dLh))
    ...
    if strcmpi(Flag,'Z')
        ...
        [dLh,resels,FWHM, nVoxels]=rp_Smoothest(StatsImgFile,MaskFile,DOF,VoxelSize);
    else
        if ~exist('Df2','var')
            Df2=0;
        end
        if strcmpi(OutputName,'DO NOT OUTPUT IMAGE')%Added by Sandy to make it compatible with Image matrix
            [Z_StatsImg P Header]=rp_TFRtoZ(StatsImgFile,OutputName,Flag,Df1,Df2,Header);
            DOF=''; %Degree of freedom for residual files
            [dLh,resels,FWHM,nVoxels]=rp_Smoothest(Z_StatsImg,MaskFile,DOF,VoxelSize);
        else
            [Z P] = rp_TFRtoZ(StatsImgFile,[OutPath,filesep,'Z_BeforeThreshold_',OutName,OutExt],Flag,Df1,Df2);
            [dLh,resels,FWHM, nVoxels]=rp_Smoothest([OutPath,filesep,'Z_BeforeThreshold_',OutName,OutExt], MaskFile);
        end
    end
end
...
if strcmpi(Flag,'Z')
    if strcmpi(OutputName,'DO NOT OUTPUT IMAGE')%Added by Sandy to make it compatible with Image matrix
        ...
        [BrainVolume, VoxelSize, Header]=rp_readfile(StatsImgFile);
    end
else
    if strcmpi(OutputName,'DO NOT OUTPUT IMAGE')%Added by Sandy to make it compatible with Image matrix
        BrainVolume=Z_StatsImg; %Transformed into Z statistic in previous step.
    else
        [BrainVolume, VoxelSize, Header]=rp_readfile([OutPath,filesep,'Z_BeforeThreshold_',OutName,OutExt]);
    end
end
...
if ~strcmpi(OutputName,'DO NOT OUTPUT IMAGE')%Added by Sandy to make it compatible with Image matrix
    rp_writefile(BrainVolume,[OutPath,filesep,'Z_ClusterThresholded_',OutName,OutExt],[nDim1,nDim2,nDim3],VoxelSize, Header,'double');
end
