function fg=use_spm_image(op,varargin)
% image and header display
% FORMAT use_spm_image
%_______________________________________________________________________
%
% use_spm_image is an interactive facility that allows orthogonal sections
...
% John Ashburner
% $Id: use_spm_image.m 3691 2010-01-20 17:08:30Z guillaume $


global st
global use_spm_image_Parameters %YAN Chao-Gan 101010
...
if nargin == 0,
    ...
    use_spm_image('init',P);
    return;
end;

if isstruct(op)
    % job data structure
    use_spm_image('init', op.data{1});
    return;
end;
...
if strcmp(op,'repos'),
    ...
    % spm_orthviews('MaxBB');
    use_spm_image('zoom_in');
    use_spm_image('update_info');
...
if strcmp(op,'addblobs'),
    ...
    for i=1:nblobs,
        ...
        set(st.blobber,'String','Remove Blobs','Callback','use_spm_image(''rmblobs'');');
    end;
...
if strcmp(op,'rmblobs'),
    ...
    set(st.blobber,'String','Add Blobs','Callback','use_spm_image(''addblobs'');');
...
if strcmp(op,'reorient'),
    ...
    mat = spm_matrix(st.B);
    use_spm_image_Parameters.ReorientMat = mat;
    %YAN Chao-Gan 101010
%     if det(mat)<=0
%         spm('alert!','This will flip the images',mfilename,0,1);
%     end;
    P = use_spm_image_Parameters.ReorientFileList;
...
%         use_spm_image('init',st.vols{1}.fname);
%     end;
    close(fg);
    return;
end;
...
if strcmp(op,'DefineROI'),
    ...
    use_spm_image_Parameters.pos=posTemp;
    close(fg);
    return;
end;
...
if ~isfield(use_spm_image_Parameters,'ReorientFileList')
    use_spm_image_Parameters.ReorientFileList={P};
end
...
spm_orthviews('MaxBB');
st.callback = 'use_spm_image(''shopos'');';
...
%-----------------------------------------------------------------------
uicontrol(fg,'Style','Frame','Position',[60 25 200 325].*WS,'DeleteFcn','use_spm_image(''reset'');');
...
uicontrol(fg,'Style','edit','Callback','use_spm_image(''repos'',1);','Position',[175 220 065 020].*WS,'String','0','ToolTipString','translate');
uicontrol(fg,'Style','edit','Callback','use_spm_image(''repos'',2);','Position',[175 200 065 020].*WS,'String','0','ToolTipString','translate');
uicontrol(fg,'Style','edit','Callback','use_spm_image(''repos'',3);','Position',[175 180 065 020].*WS,'String','0','ToolTipString','translate');
uicontrol(fg,'Style','edit','Callback','use_spm_image(''repos'',4);','Position',[175 160 065 020].*WS,'String','0','ToolTipString','rotate');
uicontrol(fg,'Style','edit','Callback','use_spm_image(''repos'',5);','Position',[175 140 065 020].*WS,'String','0','ToolTipString','rotate');
uicontrol(fg,'Style','edit','Callback','use_spm_image(''repos'',6);','Position',[175 120 065 020].*WS,'String','0','ToolTipString','rotate');
uicontrol(fg,'Style','edit','Callback','use_spm_image(''repos'',7);','Position',[175 100 065 020].*WS,'String','1','ToolTipString','zoom');
uicontrol(fg,'Style','edit','Callback','use_spm_image(''repos'',8);','Position',[175  80 065 020].*WS,'String','1','ToolTipString','zoom');
uicontrol(fg,'Style','edit','Callback','use_spm_image(''repos'',9);','Position',[175  60 065 020].*WS,'String','1','ToolTipString','zoom');

uicontrol(fg,'Style','Pushbutton','String','Reorient images','Callback','use_spm_image(''reorient'');',...
...
uicontrol(fg,'Style','Pushbutton','String','Define ROI','Callback','use_spm_image(''DefineROI'');',...
         'Position',[175 35 80 020].*WS,'ToolTipString','reset orientations of selected images');
% uicontrol(fg,'Style','Pushbutton','String','Reset...','Callback','use_spm_image(''resetorient'')',...
...
st.mp = uicontrol(fg,'Style','edit', 'Position',[110 295 135 020].*WS,'String','','Callback','use_spm_image(''setposmm'');','ToolTipString','move crosshairs to mm coordinates');
st.vp = uicontrol(fg,'Style','edit', 'Position',[110 275 135 020].*WS,'String','','Callback','use_spm_image(''setposvx'');','ToolTipString','move crosshairs to voxel coordinates');
...
st.zoomer = uicontrol(fg,'Style','popupmenu' ,'Position',[315 75 125 20].*WS,...
    ...
    'Callback','use_spm_image(''zoom_in'')','ToolTipString','zoom in by different amounts');
c = 'if get(gco,''Value'')==1, spm_orthviews(''Space''), else, spm_orthviews(''Space'', 1);end;use_spm_image(''zoom_in'')';
...
st.win = uicontrol(fg,'Style','popupmenu','Position',[315 35 125 20].*WS,...
    'String',char('Auto Window','Manual Window'),'Callback','use_spm_image(''window'');','ToolTipString','range of voxel intensities displayed');
% uicontrol(fg,'Style','pushbutton','Position',[315 35 125 20].*WS,...
%   'String','Window','Callback','use_spm_image(''window'');','ToolTipString','range of voxel intensities % displayed');
st.blobber = uicontrol(fg,'Style','pushbutton','Position',[450 35 125 20].*WS,...
    'String','Add Blobs','Callback','use_spm_image(''addblobs'');','ToolTipString','superimpose activations');
end;
return;
