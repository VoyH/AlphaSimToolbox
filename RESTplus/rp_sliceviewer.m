...
if nargin<1, %No param Launch, 20070918
	%help(mfilename);
	if nargout>=1,
		varargout{1}=rp_sliceviewer('ShowImage','');	%by Default, I show a black brain image
	else
		rp_sliceviewer('ShowImage','');	%by Default, I show a black brain image
	end
	return; 
end
...
%try
	...
	case 'SHOWIMAGE',	%ShowImage
		if nargin==2,
			%rp_sliceviewer('ShowImage', theBrainMap); %reho_gui.m 989
			AFilename =varargin{1};
			ACallback ='';		
	    elseif nargin==3,
			%rp_sliceviewer('ShowImage', theBrainMap, [theCallback cmdClearVar], 'Power Spectrum');%reho_gui.m 1010
			AFilename =varargin{1};
			ACallback =varargin{2};		
		else
			error(sprintf('Usage: hFig =rp_sliceviewer(''ShowImage'', AFilename); \n\t hFig =rp_sliceviewer(''ShowImage'', AFilename, ACallback);')); 
		end
	...
	case 'UPDATECALLBACK', 		%UpdateCallback
		...
		elseif nargin==4, 
			...
			error('Usage: rp_sliceviewer(''UpdateCallback'', AFigureHandle, ACallback, ACallbackCaption);'); 
		end
	...
	case 'DELETE', % Delete specific figure
		if nargin~=2, error('Usage: rp_sliceviewer(''Delete'', AFigHandle);'); end
		AFigHandle =varargin{1};
		REST_SliceViewer_Cfg =DeleteFigure(REST_SliceViewer_Cfg, AFigHandle);
		
		if ~GetDisplayCount(REST_SliceViewer_Cfg),
			rp_sliceviewer('QuitAllSliceViewer');
		end
		
	case {'CLICKPOSITION', 'SETPOSITION'},		%ClickPosition	%SetPosition
		if nargin~=2, error('Usage: rp_sliceviewer(''ClickPosition'', AFigHandle); or rp_sliceviewer(''SetPosition'', AFigHandle);'); end
		AFigHandle =varargin{1};
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0
			...
			if isSelfYoked ,
				...
				for x=1:GetDisplayCount(REST_SliceViewer_Cfg), 
					if x~=theCardinal ,
						...
						if isYoked, 							
							rp_sliceviewer('SetPhysicalPosition', REST_SliceViewer_Cfg.Config(x).hFig, theDistanceFromOrigin);
						end
					end
				end
			end	
			...
			rp_sliceviewer('RunCallback', AFigHandle);			
		end
		
	case 'SETPHYSICALPOSITION',		%SetPhysicalPosition, for Yoke, the physical position(mm) from the origin
		if nargin~=3, error('Usage: rp_sliceviewer(''SetPhysicalPosition'', AFigHandle, ADistanceFromOrigin);'); end
		AFigHandle =varargin{1};
	...
	case 'RUNCALLBACK', 			%RunCallback	, 20070625
		if nargin~=2, error('Usage: rp_sliceviewer(''RunCallback'', AFigHandle);'); end
		AFigHandle =varargin{1};	
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0		
			% Run the Callback
	...
	case 'GETPOSITION', 		%GetPosition, return Absolute	position where current cross-hair stay
		if nargin~=2, error('Usage: Position =rp_sliceviewer(''GetPosition'', AFigHandle);'); end
		AFigHandle =varargin{1};
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0
	...
	case 'GETPHYSICALPOSITION', 		%GetPhysicalPosition return Coordinate .* VoxelSize	(mm)
		if nargin~=2, error('Usage: DistanceFromOrigin =rp_sliceviewer(''GetPhysicalPosition'', AFigHandle);'); end
		AFigHandle =varargin{1};
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0
	...
	case 'SETMESSAGE',			%SetMessage, update the Message, I don't contain the Copyright any more , 20070915
		if nargin~=3, error('Usage: rp_sliceviewer(''SetMessage'', AFigHandle, AMessage);'); end
		AFigHandle 	=varargin{1};
		AMessage	=varargin{2};
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0
	...
	case 'MAGNIFY', 		%Magnify, x0.5 -x1 -x2 -x3 -x4 -x5 -x6
		if nargin~=2, error('Usage: rp_sliceviewer(''Magnify'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0
	...
	case 'QUITALLSLICEVIEWER',		%QuitAllSliceViewer
		if nargin~=1, error('Usage: rp_sliceviewer(''QuitAllSliceViewer'');'); end
		for x=GetDisplayCount(REST_SliceViewer_Cfg):-1:1, % DawnSong, revised 20070625
			rp_sliceviewer('DELETE', REST_SliceViewer_Cfg.Config(x).Filename);
		end
		if mislocked(mfilename),munlock; end
		clear REST_SliceViewer_Cfg;
		
	case 'MONTAGE', 		%Montage	
		if nargin~=2, error('Usage: rp_sliceviewer(''Montage'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0,
	...
	case 'ORTHOGONALVIEW', 		%OrthogonalView
		if nargin~=2, error('Usage: rp_sliceviewer(''OrthogonalView'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0,
	...
	case 'TRANSVERSEVIEW', 		%TransverseView
		if nargin~=2, error('Usage: rp_sliceviewer(''TransverseView'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0,
	...
	case 'SAGITTALVIEW', 		%SagittalView
		if nargin~=2, error('Usage: rp_sliceviewer(''SagittalView'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0,
	...
	case 'CORONALVIEW', 		%CoronalView
		if nargin~=2, error('Usage: rp_sliceviewer(''CoronalView'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0,
	...
	case 'REPAINT',		%Repaint	
		if nargin~=2, error('Usage: rp_sliceviewer(''Repaint'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0,		
			REST_SliceViewer_Cfg.Config(theCardinal) =SetImage(REST_SliceViewer_Cfg.Config(theCardinal));
	...
	case 'MNI/TALAIRACH',		%MNI/Talairach
		...
		if theCardinal>0,		
			...
			rp_sliceviewer('SetPosition', AFigHandle);
		end;
		
	case 'CHANGEUNDERLAY', 		%ChangeUnderlay
		if nargin~=2, error('Usage: rp_sliceviewer(''ChangeUnderlay'', AFigHandle);'); end
		AFigHandle 	=varargin{1};
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0,
	...
	case 'UNDERLAYSELECTION', 		%UnderlaySelection	
		if nargin~=2, error('Usage: rp_sliceviewer(''UnderlaySelection'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0,			
			...
			if any(filename~=0) && ischar(filename) && length(filename)>4 ,	% not canceled and legal			
				...
				rp_sliceviewer('ChangeUnderlay', AFigHandle);
			end			
		end;
	case 'CLICKRECENTUNDERLAY', 	%ClickRecentUnderlay
		if nargin~=2, error('Usage: rp_sliceviewer(''ClickRecentUnderlay'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0,			
	...
	case 'CHANGEOVERLAY', 		%ChangeOverlay
		if nargin~=2, error('Usage: rp_sliceviewer(''ChangeOverlay'', AFigHandle);'); end
		AFigHandle 	=varargin{1};
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0,
    ...
    case 'CHANGETIMEPOINT',
        if nargin~=2, error('Usage: rp_sliceviewer(''ChangeTimePoint'',AFigHandle);');end
        AFigHandle =varargin{1};
        theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
        if theCardinal>0,
    ...
    case 'CHANGETIMEPOINTLEFT',
        if nargin~=2, error('Usage: rp_sliceviewer(''ChangeTimePointLeft'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
        if theCardinal>0
            ...
            rp_sliceviewer('ChangeTimePoint',AFigHandle);
        end
        
    case 'CHANGETIMEPOINTRIGHT',
        if nargin~=2, error('Usage: rp_sliceviewer(''ChangeTimePointRight'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
        if theCardinal>0
            TimePoint=str2num(get(REST_SliceViewer_Cfg.Config(theCardinal).hVolumeIndexText,'String'));
	...
	case 'OVERLAYSELECTION', 		%OverlaySelection	
		%if nargin~=2, error('Usage: rp_sliceviewer(''UnderlaySelection'', AFigHandle);'); end
        if nargin~=2, error('Usage: rp_sliceviewer(''OverlaySelection'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0,		
			...
			if any(filename~=0) && ischar(filename) && length(filename)>4 ,	% not canceled and legal			
				...
				rp_sliceviewer('ChangeOverlay', AFigHandle);
     ...
     case 'CLICKRECENTOVERLAY', 	%ClickRecentOverlay
		if nargin~=2, error('Usage: rp_sliceviewer(''ClickRecentOverlay'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0,			
	...
	case 'OVERLAY_SETTHRDABSVALUE',		%Overlay_SetThrdAbsValue, %Change from the Absolute value Edit control
		if nargin~=2, error('Usage: rp_sliceviewer(''Overlay_SetThrdAbsValue'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0,
	...
	case 'OVERLAY_SETTHRDCLUSTERSIZE', 		%Overlay_SetThrdClusterSize
		if nargin~=2, error('Usage: rp_sliceviewer(''Overlay_SetThrdClusterSize'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0,
	...
	case 'OVERLAY_MISC', 		%Overlay_Misc
		if nargin~=2, error('Usage: rp_sliceviewer(''Overlay_Misc'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0,
			REST_SliceViewer_Cfg.Config(theCardinal) =Overlay_Misc(REST_SliceViewer_Cfg.Config(theCardinal));
			rp_sliceviewer('Repaint', AFigHandle);
		end	
    ...
    case 'CLUSTERSREPORT'
        if nargin~=2, error('Usage: rp_sliceviewer(''Clusters Report'', AFigHandle);'); end
	...
	case 'CURRENTTHRD2MASK',	%dong save thrd2mask 2009-09-09
        if nargin~=2, error('Usage: rp_sliceviewer(''CurrentThrd2Mask'', AFigHandle);'); end
        AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
        if theCardinal>0,
			theConfig =REST_SliceViewer_Cfg.Config(theCardinal);
	...
	case 'CURRENTCLUSTER2MASK',			%CurrentCluster2Mask
		if nargin~=2, error('Usage: rp_sliceviewer(''CurrentCluster2Mask'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0,
			theConfig =REST_SliceViewer_Cfg.Config(theCardinal);
	...
	case 'SAVERECENT',		%SaveRecent		
		if nargin~=2, error('Usage: rp_sliceviewer(''SaveRecent'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0,
	...
	case 'CHANGECOLORELEMENT', 		%ChangeColorElement
		if nargin~=2, error('Usage: rp_sliceviewer(''ChangeColorElement'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0,
	...
	case 'TOGGLEINFODISPLAY', 		%ToggleInfoDisplay
		if nargin~=2, error('Usage: rp_sliceviewer(''ToggleInfoDisplay'', AFigHandle);'); end
		AFigHandle 	=varargin{1};	
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0,
	...
	case 'UPDATECALLBACK_SAVE2MASK',	%UpdateCallback_Save2Mask
		if nargin~=3, error('Usage: rp_sliceviewer(''UpdateCallback_Save2Mask'', AFigHandle, ACallback);'); end
		AFigHandle 	=varargin{1};	
		ACallback   =varargin{2};	
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0,
	...
	case 'GETSAVEDMASKFILENAME',		%GetSavedMaskFilename
		if nargin~=2, error('Usage: rp_sliceviewer(''GetSavedMaskFilename'', AFigHandle);'); end
		AFigHandle 	=varargin{1};			
		theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
		if theCardinal>0,
	...
	case 'SHOWOVERLAY',	%ShowOverlay
		if nargin>3, error('Usage: rp_sliceviewer(''ShowOverlay'', AOverlay); or rp_sliceviewer(''ShowOverlay'', AFigHandle, AOverlay);'); end
           AFigHandle 	=varargin{1};
            if ~ischar(AFigHandle),
                ...
                AOverlay    =varargin{1};
                theFig = rp_sliceviewer; %I can't write "()" considering Matlab 6.5 compatiable
                rp_sliceviewer('ShowOverlay', theFig, AOverlay);
                varargout{1} =theFig;
            end
        
     
        case 'SHOWUNDERLAY'    % By Gao Zhong zhan 20150330 and revised by Jia Xi-Ze  151107
            
         if nargin>3, error('Usage: rp_sliceviewer(''ShowUnderlay'', AUnderlay); or rp_sliceviewer(''ShowUnderlay'', AFigHandle, AUnderlay);'); end
            AFigHandle=varargin{1};
            if ~ischar(AFigHandle),
                AUnderlay    =varargin{2};
                theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
                if theCardinal>0,
                    set(REST_SliceViewer_Cfg.Config(theCardinal).hUnderlayFile, 'String', AUnderlay);
                    rp_sliceviewer('ChangeUnderlay', AFigHandle);
                end
            else	%I will create a new slice-viewer to show a undelay
                AUnderlay    =varargin{1};
                theFig = rp_sliceviewer; %I can't write "()" considering Matlab 6.5 compatiable
                rp_sliceviewer('ShowUnderlay', theFig, AUnderlay);
                varargout{1} =theFig;
            end         
        ...
        case 'ONKEYPRESS',		%OnKeyPress
            if nargin~=2, error('Usage: rp_sliceviewer(''OnKeyPress'', AFigHandle);'); end
            AFigHandle 	=varargin{1};
            theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
            if theCardinal>0,
        ...
        case 'SCROLLWHEELDOWN',		%dong 100520
            if nargin~=2, error('Usage: rp_sliceviewer(''ScrollWheel'', AFigHandle);'); end
            AFigHandle 	=varargin{1};
            theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
            if theCardinal>0,
        ...
        case 'CHANGEMAXPOSITIVE'%ChangeMaxPositive Sandy Wang 20120419
            if nargin~=2, error('Usage: rp_sliceviewer(''ChangeMaxPositive'', AFigHandle);'); end
            AFigHandle 	=varargin{1};
            theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
            if theCardinal>0
        ...
        case 'CHANGEMINNEGATIVE'%ChangeMaxPositive Sandy Wang 20120419
            if nargin~=2, error('Usage: rp_sliceviewer(''ChangeMinNegative'', AFigHandle);'); end
            AFigHandle 	=varargin{1};
            theCardinal =ExistViewer(REST_SliceViewer_Cfg, AFigHandle);
            if theCardinal>0
...
function Result =InitControls(AFilename, ACallback)	
	...
	set(theFig, 'DeleteFcn', sprintf('rp_sliceviewer(''Delete'', %g);', theFig)  );
	set(theFig, 'KeyPressFcn', sprintf('rp_sliceviewer(''OnKeyPress'', %g);', theFig)  );
    ...
    %if rp_misc('GetMatlabVersion')>=7.4%100520 dong Matlab 7.4 begin
	...
	hCrosshair =uicontrol(theFig, 'Style','checkbox', 'Units','pixels', ...
					...
					'Callback', sprintf('rp_sliceviewer(''Repaint'', %g);',theFig) , ...
					'BackgroundColor', get(theFig,'Color'), ...
					'Position',[theLeft, theBottom+75, 70,15]);
	
	
	theEditCallbackFcn =sprintf('rp_sliceviewer(''SetPosition'', %g);', theFig);
	...
	hMagnify =uicontrol(theFig, 'Style','popupmenu', 'Units','pixels', ...
						...
						'BackgroundColor', get(theFig,'Color'), ...
						'Enable', 'off', ...
						'Callback', sprintf('rp_sliceviewer(''Magnify'', %g);',theFig), ...
	...
	hMniTal =uicontrol(theFig, 'Style','popupmenu', 'Units','pixels', ...
						...
						'BackgroundColor', get(theFig,'Color'), ...
						'Enable', 'on', ...
						'Callback', sprintf('rp_sliceviewer(''MNI/Talairach'', %g);',theFig), ...
	...
	hMsgLabel =uicontrol(theFig, 'Style','text', 'Units','pixels', ...
			  ...
			  'ButtonDownFcn', sprintf('rp_sliceviewer(''ToggleInfoDisplay'', %g);',theFig), ...
	...
	hViewTransverse =uicontrol(theFig, 'Style','pushbutton',  ...
							  'Units','pixels', ...
							  'Callback', sprintf('rp_sliceviewer(''TransverseView'', %g);',theFig) , ...
							  'CData', theIcon,...
	...
	hViewSagittal =uicontrol(theFig, 'Style','pushbutton',  ...
							  'Units','pixels', ...
							  'Callback', sprintf('rp_sliceviewer(''SagittalView'', %g);',theFig) , ...
							  'CData', theIcon,...
							  'Position',[theLeft, theBottom, 34,36]);
	...
	hViewCoronal =uicontrol(theFig, 'Style','pushbutton',  ...
							  'Units','pixels', ...
							  'Callback', sprintf('rp_sliceviewer(''CoronalView'', %g);',theFig) , ...
							  'CData', theIcon,...
							  'Position',[theLeft, theBottom, 34,36]);
	...
	hViewOrthogonal =uicontrol(theFig, 'Style','pushbutton',  ...
							  'Units','pixels', ...
							  'Callback', sprintf('rp_sliceviewer(''OrthogonalView'', %g);',theFig) , ...
							  'CData', theIcon,...
							  'Position',[theLeft, theBottom, 34,36]);
	...
	hViewMontage =uicontrol(theFig, 'Style','pushbutton',  ...
							  'Units','pixels', ...
							  'Callback', sprintf('rp_sliceviewer(''Montage'', %g);',theFig) , ...
    ...
    hUnderlayRecent =uicontrol(theFig, 'Style','popupmenu', 'Units','pixels', ...
			  ...
			  'Callback', sprintf('rp_sliceviewer(''ClickRecentUnderlay'', %g);',theFig), ...
			  'BackgroundColor', get(theFig,'Color'), ...			  
	...
	hUnderlayFile =uicontrol(theFig, 'Style','edit', 'Units','pixels', ...
							  'String', '', ...
							  'BackgroundColor', 'white', ...
							  'Callback', sprintf('rp_sliceviewer(''ChangeUnderlay'', %g);',theFig), ...
	...
	hULSel=uicontrol(theFig, 'Style','pushbutton', 'Units','pixels', ...
		  'Callback', sprintf('rp_sliceviewer(''UnderlaySelection'', %g);',theFig) , ...
	...
	hOverlayRecent =uicontrol(theFig, 'Style','popupmenu', 'Units','pixels', ...
			  ...
			  'Callback', sprintf('rp_sliceviewer(''ClickRecentOverlay'', %g);',theFig), ...
			  'BackgroundColor', get(theFig,'Color'), ...			  
			  'Position',[theLeft, theBottom, 75,20]);
    ...
    hVolumeIndexLeft =uicontrol(theFig, 'Style','pushbutton','Units','pixels',...
                              'Callback',sprintf('rp_sliceviewer(''ChangeTimePointLeft'',%g);',theFig),...
                              'String','<','FontWeight','bold',...
    ...
    hVolumeIndexText =uicontrol(theFig, 'Style','edit','Units','pixels',...
                              'String', '',...
                              'BackgroundColor','White',...
                              'Callback',sprintf('rp_sliceviewer(''ChangeTimePoint'',%g);',theFig),...
    ...
    hVolumeIndexRight =uicontrol(theFig, 'Style','pushbutton','Units','pixels',...
                              'Callback',sprintf('rp_sliceviewer(''ChangeTimePointRight'',%g);',theFig),...
                              'String','>','FontWeight','bold',...
                              'Position',[theLeft,theBottom,10,20]);
	...
	hOverlayFile =uicontrol(theFig, 'Style','edit', 'Units','pixels', ...
					  'String', '', ...					  
					  'BackgroundColor', 'white', ...
					  'Callback', sprintf('rp_sliceviewer(''ChangeOverlay'', %g);',theFig), ...
					  'Position',[theLeft, theBottom, 150,20]);
	...
	hOLSel=uicontrol(theFig, 'Style','pushbutton', 'Units','pixels', ...
		  'Callback', sprintf('rp_sliceviewer(''OverlaySelection'', %g);',theFig) , ...
		  'String', '...','FontWeight', 'bold', ...
		  'Position',[theLeft, theBottom, 26,15]); 
	...
	hEdtThrdValue =uicontrol(theFig, 'Style','edit', 'Units','pixels', ...
					'String', '', ...					
					'Callback', sprintf('rp_sliceviewer(''Overlay_SetThrdAbsValue'', %g);',theFig), ...	
					 'BackgroundColor', 'white', ...
    ...
    hdf =uicontrol(theFig, 'Style','pushbutton',  ... %add df dong 090921
							  'Units','pixels', ...
							  'Callback', sprintf('rp_sliceviewer(''DF'', %g);',theFig),...	
							  'String', 'df', ...
	...
	if ~ispc && ~ismac%Added by Sandy to fix a display at UNIX
        hOverlayMisc =uicontrol(theFig, 'Style','popupmenu', 'Units','pixels', ...
					...
					'BackgroundColor', get(theFig,'Color'), ...
					'Enable', 'on', ...
					'Callback', sprintf('rp_sliceviewer(''Overlay_Misc'', %g);',theFig), ...
       ...
       hSaveClusters=uicontrol(theFig, 'Style','pushbutton', 'Units','pixels', ...%dong 090920
			...
			'Callback', sprintf('rp_sliceviewer(''CurrentThrd2Mask'', %g);',theFig),...			
			'Position',[theLeft+5, theBottom-15, 70,24]);
       hClReport=uicontrol(theFig, 'Style','pushbutton', 'Units','pixels', ...%dong 090920
			'String', 'Cl. Report', ...
            'Callback', sprintf('rp_sliceviewer(''ClustersReport'', %g);',theFig),...				
	...
	%		 'String', 'Thrd2Mask', ...
	%		 'Callback', sprintf('rp_sliceviewer(''Overlay_SetThrdClusterSize'', %g);',theFig),...			
	%		 'Position',[theLeft+75, theBottom+5, 70,24]);
			
	%Colorbar for overlay, do as AFNI, 20070921
	theAxesButtonDownFcn =sprintf('rp_sliceviewer(''ChangeColorElement'', %g);',theFig);
...
function Result =DeleteFigure(AGlobalConfig, AFigHandle)
	...
	if x>0,			
		...
		if isExistFig,
			...
			rp_sliceviewer('SaveRecent', AFigHandle);
...
function Result =SetImage(AConfig)
	...
	set(AConfig.hFig, 'Name', sprintf('Slice Viewer -- REST %s         %s',rp_misc( 'GetRestVersion'),AConfig.Overlay.Filename));
...
function SetMessage(AConfig)
	...
	if SeeOverlay(AConfig),
		...
		if all(theOverlayPos<=size(AConfig.Overlay.Volume)) ...
			...
			rp_sliceviewer('SetPhysicalPosition', AConfig.hFig, [0 0 0]);
			rp_sliceviewer('Repaint', AConfig.hFig);
			return;
		end
...
function Result =UpdateCallback(AConfig, ACallback, ACallbackCaption)
	...
	% else
		...
		% if ~isempty(ACallback) && (ischar(ACallback) || isa(ACallback, 'function_handle')),
			% theCallback =sprintf('rp_sliceviewer(''RunCallback'', ''%s'');',AConfig.Filename);
...
function Transforming_MNI_Talairach(AConfig)
	...
	case 2, %'From Talairach to MNI'
        ...
        thePosition =round(rp_tal2icbm_spm(AConfig.LastPosition -AConfig.Origin) +AConfig.Origin);
        AConfig =UpdatePosition(AConfig, thePosition);
		isNeedUpdate =true;
	case 3, %'From MNI to Talairach'
        ...
        thePosition=round(rp_icbm_spm2tal(AConfig.LastPosition -AConfig.Origin) +AConfig.Origin);
		AConfig =UpdatePosition(AConfig, thePosition);
		isNeedUpdate =true;
	otherwise
	end
	...
	if isNeedUpdate,
		ShowPositionInEdit(AConfig);
		SetMessage(AConfig);
		rp_sliceviewer('SetPosition', AConfig.Filename);
	end
...
%Input: Need AConfig.Filename specified	
	...
	theAxesButtonDownFcn =sprintf('rp_sliceviewer(''ClickPosition'', %g);', AConfig.hFig);
...
function Result =LoadOverlay(AConfig, AFilename)
	...
	try
        ...
        if exist(AFilename,'dir')==7
            [AllVolume,theVoxelSize,ImgFileList,Header]=rp_to4d(AFilename);
        else
            [AllVolume,theVoxelSize, Header] =rp_readfile(AFilename); %%Yan 080610
        end
...
%             
            ...
            H_BrainNet = rp_CallBrainNetViewer(AConfig.Overlay.VolumeThrd,-AConfig.Overlay.ValueThrdAbsolute,AConfig.Overlay.ValueThrdAbsolute,0,18,SurfFileName,viewtype,AConfig.Overlay.Colormap,NMax,PMax,AConfig.Overlay.Header);
            %H_BrainNet = rp_CallBrainNetViewer(BrainVolume,NMin,PMin,ClusterSize,ConnectivityCriterion,SurfFileName,viewtype,ColorMap,NMax,PMax,BrainHeader)
        case 5, %Save Image 
			%YAN Chao-Gan 081223: add "save image as" function
...
%                 },'Correction Thresholds by AlphaSim');
        ...
        case 9 % DONG FDR 100118
            ...
            [Qvalue,Qmaskname,Conproc,Tchoose]=rp_FDR_gui(AConfig.Overlay.Qvalue,AConfig.Overlay.Qmaskname,AConfig.Overlay.Conproc,AConfig.Overlay.Tchoose);
            ...
            case 10%Gaussian Random Field (GRF) Correction added by Sandy
                [mask_file,voxel_p,cluster_p,Is_two_tail]=rp_GRF_for_sliceview;
                if isempty(voxel_p)
                    Result =AConfig;
                    return;
                else
                    ...
                    [Data_Corrected, ClusterSize, Header]=rp_GRF_Threshold(AConfig.Overlay.Volume,...
                                                    voxel_p,...
                                                    Is_two_tail,...
                                                    cluster_p,...
                                                    'DO NOT OUTPUT IMAGE',...
...
function Result =Open_Template(AConfig)	
	...
	case 2, %'Open Template AAL'
		...
		% rp_sliceviewer('ChangeOverlay', AConfig.hFig);
...
% 			end
			if x==1 %&& thePercentValue~=0
                hTextMinNegative=uicontrol(AConfig.hFig, 'Style','edit', 'Units','pixels', ...
					  ...
					  'BackgroundColor', 'white', ...
                      'Callback', sprintf('rp_sliceviewer(''ChangeMinNegative'', %g);',AConfig.hFig),...
...
% 			end
            ...
            if x==theNumberCmd && thePercentValue~=0
                hTextMaxPositive=uicontrol(AConfig.hFig, 'Style','edit', 'Units','pixels', ...
					  ...
					  'BackgroundColor', 'white', ...
                      'Callback', sprintf('rp_sliceviewer(''ChangeMaxPositive'', %g);',AConfig.hFig),...
...
if (evnt.VerticalScrollCount>0),
    % scroll down
    eval(sprintf('rp_sliceviewer(''ScrollWheelDown'', %g);', src));
else
    % scroll up
    eval(sprintf('rp_sliceviewer(''ScrollWheelUp'', %g);', src));
end;
